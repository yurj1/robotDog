...
#include "manager/unitree_sdk_service.h"
...
//           }
    ...
    // 处理指令消息
    void RobotDogMain::orderExec(const robot_dog::Order& msg) {
      if(message_handle_manager_) message_handle_manager_->handleOrderEvent(msg);
    }
    void RobotDogMain::HandleObuCmdMsg(const std::vector<robot_dog::ObuCmd>& obu_cmd_msg)
    {
      if(message_handle_manager_) message_handle_manager_->HandleObuCmdMsg(obu_cmd_msg);
    }
  }
}