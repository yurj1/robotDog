  ...
  namespace function 
  {
    ...
    void RobotDogMain::Start()
    {
      ...
      // step5 读取配置文件
      {
        ...
        {
          nlohmann::json RJson;
          RJson["pointsTude"] = roobt_dog_json_["pointsTude"] ;
          RJson["recommendedRoute"] = roobt_dog_json_["recommendedRoute"];
          AINFO << "pointsTude:" << RJson["pointsTude"].dump();
          AINFO << "recommendedRoute:" << RJson["recommendedRoute"].dump();
          if(_AppGetMqttService) {
            AINFO << "publish recommendedRoute";
            std::thread([RJson](){
              std::this_thread::sleep_for(std::chrono::milliseconds(500));
              _AppGetMqttService->PublishOutdoorRecommendedRoute(RJson.dump());
            }).detach();
          }
        }
    ...
    void RobotDogMain::PublishVideoOnInt(const robot_dog::ObuCmdMsg& msg)
    {
#if LCM_ENABLE
      if (message_manager_.count("LCM") > 0)
