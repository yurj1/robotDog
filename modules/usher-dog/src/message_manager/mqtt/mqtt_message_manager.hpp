...
using  Json = nlohmann::json;
 namespace athena
 {
  namespace function {
    ...
    //  }
     template <typename T>
     void MqttMessageManager<T>::PublishTaskList(robot_dog::TaskList msg) {
       //_pubscriber[pub_perception_mode].publish(msg);
     }
     template <typename T>
     void MqttMessageManager<T>::PublishPose(robot_dog::Position msg) {
       //_pubscriber[pub_goal_state_extern].publish(msg);
     }
     template <typename T>
     void MqttMessageManager<T>::PublishState(robot_dog::PercState msg) {
       //_pubscriber[pub_feedback_to_cmd].publish(msg);
     }
     
     template <typename T>
     void MqttMessageManager<T>::PublishAction(robot_dog::ActionEntry msg) {
    ...
    void MqttMessageManager<T>::HandleJoyMsg(const std::string& msg)
    {
      ...
      if(_AppIsJoyServiceNotNull)
      _AppGetJoyService->HandleJoyMsg(result);
    }
    ...
    template <typename T>
    std::string MqttMessageManager<T>::GenerateClientId(const std::string& baseName) 
    {
    auto now_ms = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::system_clock::now().time_since_epoch()
    ).count();
    return baseName + "_" + std::to_string(now_ms);
    }
