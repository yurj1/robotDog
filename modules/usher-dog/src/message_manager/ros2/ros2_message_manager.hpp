...
namespace athena
{
  namespace function
  {
    ...
    using TaskList = perception_msgs::msg::TaskList;
    using PercState = perception_msgs::msg::PercState;
    using ActionEntry = perception_msgs::msg::ActionEntry;
    using PercCmd = perception_msgs::msg::PercCmd;

    template <typename T>
    Ros2MessageManager<T>::Ros2MessageManager() : Node{"robot_dog_app"} {}
    template <typename T>
    void Ros2MessageManager<T>::Init(T *t)
    {
      ...
      task_msg_output_pub_ = create_publisher<TaskList>(
        pub_perception_mode, QoS{10}, PubAllocT{});
      rviz_target_point_msg_output_pub_ = create_publisher<geometry_msgs::msg::Pose>(
        pub_goal_state_extern, QoS{10}, PubAllocT{});
      feedback_to_cmd_output_pub_ = create_publisher<PercState>(
        pub_feedback_to_cmd, QoS{10}, PubAllocT{});
      action_to_cmd_output_pub_ = create_publisher<ActionEntry>(
        pub_action_info_to_cmd, QoS{10}, PubAllocT{});

      task_sub_ = create_subscription<PercCmd>(
        sub_callback_to_cmd, QoS{10},
        [this](const perception_msgs::msg::PercCmd::SharedPtr msg)
        {
          cmdCallback(msg);
        },
        SubAllocT{});

      callback_perception_sub_ = create_subscription<TaskList>(
        sub_recv_callback_perception, QoS{10},
        [this](const perception_msgs::msg::TaskList::SharedPtr msg)
        {
          ptCallback(msg);
        },
        SubAllocT{});

      callback_planning_sub_ = create_subscription<TaskList>(
      ...
      // order_exec_sub_ = create_subscription<Order>(
      //   sub_planning_feedback, QoS{10},
      //   [this](const perception_msgs::msg::Order::SharedPtr msg)
      //   {
      //     orderExec(msg);
      //   },
      //   SubAllocT{});
    ...
    template <typename T>
    void Ros2MessageManager<T>::PublishTaskList(robot_dog::TaskList msg) {
      //_pubscriber[pub_perception_mode].publish(msg); task_msg_output_pub_
      TaskList task_list;
      task_list.task_id = msg.task_id;
      task_list.task_type = msg.task_type;

      task_list.target_position.position.x = msg.target_position.x;
      task_list.target_position.position.y = msg.target_position.y;
      task_list.target_position.position.z = msg.target_position.z;
      task_list.target_position.orientation.x = msg.target_position.q_x;
      task_list.target_position.orientation.y = msg.target_position.q_y;
      task_list.target_position.orientation.z = msg.target_position.q_z;
      task_list.target_position.orientation.w = msg.target_position.q_w;

      task_list.target_object = msg.target_object;
      task_list.task_state = msg.task_state;
      task_list.task_result = msg.task_result;
      task_list.is_in_place_rotation = msg.is_in_place_rotation;

      task_msg_output_pub_->publish(task_list);
    }
    template <typename T>
    void Ros2MessageManager<T>::PublishPose(robot_dog::Position msg) {
      ...
      rviz_target_point_msg_output_pub_->publish(pose);
    }
    template <typename T>
    void Ros2MessageManager<T>::PublishState(robot_dog::PercState msg) {
      PercState perc_state;
      ...
      feedback_to_cmd_output_pub_->publish(perc_state);
    }

    template <typename T>
    void Ros2MessageManager<T>::PublishAction(robot_dog::ActionEntry msg) {
      ActionEntry action_entry;
      action_entry.id = msg.id;
      action_entry.param1= msg.param1;
      action_entry.param2= msg.param2;
      action_entry.param3= msg.param3;
      action_entry.info= msg.info;

      action_to_cmd_output_pub_->publish(action_entry);
    }
    ...
    template <typename T>
    void Ros2MessageManager<T>::cmdCallback(const perception_msgs::msg::PercCmd::SharedPtr msg) {
    ...
    template <typename T>
    void Ros2MessageManager<T>::ptCallback(const perception_msgs::msg::TaskList::SharedPtr msg) {
    ...
    template <typename T>
    void Ros2MessageManager<T>::stateCallback(const perception_msgs::msg::TaskList::SharedPtr msg) {
      robot_dog::TaskList task_list;
    ...
    // template <typename T>
    // void Ros2MessageManager<T>::orderExec(const perception_msgs::msg::Order::SharedPtr msg) {
    //   robot_dog::Order order;
    //   instance_->orderExec(order);
    // }
