...
namespace athena
{
  namespace function
  {
    ...
    bool RobotDogMain::RobotDogMainStateMachineInit()
    {
      ...
      robot_dog_conf_sm->SetCallback(state_machine::CallbackType::UPDATE, "UsherDog",
    ...
    void RobotDogMain::IdleStateUpdate(const std::string &state_name, int state)
    {
      ...
      if (message_handle_manager_->GetDogState() == UsherDogState::DOG_USHER)
      {
        AINFO << "change : RobotDogMainUsherRun ";
        robot_dog_conf_sm->NextState("UsherRun");
        return;
      }
    }
    ...
    {
      //AINFO << "state_machine : UsherRun";

      if (message_handle_manager_->GetDogState() == UsherDogState::DOG_FINISH)
      {
        AINFO << "change : RobotDogMainComplete ";
        robot_dog_conf_sm->NextState("complete");
        return;
      }
      else if (message_handle_manager_->GetDogState() == UsherDogState::DOG_IDLE)
      {
        // AINFO << "change : RobotDogMainIdle ";
        // robot_dog_conf_sm->NextState("Idle");
        return;
    ...
    void RobotDogMain::FinishStateUpdate(const std::string &state_name, int state)
    {
      ...
      if (message_handle_manager_->GetDogState() == UsherDogState::DOG_IDLE)
      {
        AINFO << "change : RobotDogMainIdle ";
        robot_dog_conf_sm->NextState("Idle");
        return;
