...
class ObuCmdMsg {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.obu_cmd_list.forEach((val) => {
      bufferOffset = ObuCmd.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.obu_cmd_list[i] = ObuCmd.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static datatype() {
    ...
    return 'ros_interface/ObuCmdMsg';
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new ObuCmdMsg(null);
    ...
    if (msg.obu_cmd_list !== undefined) {
      ...
      for (let i = 0; i < resolved.obu_cmd_list.length; ++i) {
        resolved.obu_cmd_list[i] = ObuCmd.Resolve(msg.obu_cmd_list[i]);
      }
    }
    else {
