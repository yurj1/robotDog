...
class HMIDiagnosticArray {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.status.forEach((val) => {
      bufferOffset = HMIDiagnosticStatus.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    let len;
    let data = new HMIDiagnosticArray(null);
    ...
    for (let i = 0; i < len; ++i) {
      data.status[i] = HMIDiagnosticStatus.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static getMessageSize(object) {
    ...
    object.status.forEach((val) => {
      length += HMIDiagnosticStatus.getMessageSize(val);
    });
    return length + 4;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new HMIDiagnosticArray(null);
    ...
    if (msg.status !== undefined) {
      ...
      for (let i = 0; i < resolved.status.length; ++i) {
        resolved.status[i] = HMIDiagnosticStatus.Resolve(msg.status[i]);
      }
    }
    else {
      resolved.status = []
...
module.exports = HMIDiagnosticArray;
