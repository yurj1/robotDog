 ...
 *	S p a r s e S o l v e r
 */
SparseSolver::SparseSolver(const SparseSolver& rhs) { copy(rhs); }

/*
 *	~ S p a r s e S o l v e r
 */
SparseSolver::~SparseSolver() { clear(); }
 ...
 *	r e s e t
 */
returnValue SparseSolver::reset() { return SUCCESSFUL_RETURN; }

/*
 *	g e t N e g a t i v e E i g e n v a l u e s
 */
int_t SparseSolver::getNegativeEigenvalues() { return -1; }

/*
 *	g e t R a n k
 */
int_t SparseSolver::getRank() { return -1; }
 ...
 *	c l e a r
 */
returnValue SparseSolver::clear() { return SUCCESSFUL_RETURN; }
...
void MA27AD(fint_t* N, fint_t* NZ, const fint_t* IRN, const fint_t* ICN,
            fint_t* IW, fint_t* LIW, fint_t* IKEEP, fint_t* IW1, fint_t* NSTEPS,
            fint_t* IFLAG, fint_t* ICNTL, double* CNTL, fint_t* INFO,
            double* OPS);
void MA27BD(fint_t* N, fint_t* NZ, const fint_t* IRN, const fint_t* ICN,
            double* A, fint_t* LA, fint_t* IW, fint_t* LIW, fint_t* IKEEP,
            fint_t* NSTEPS, fint_t* MAXFRT, fint_t* IW1, fint_t* ICNTL,
            double* CNTL, fint_t* INFO);
void MA27CD(fint_t* N, double* A, fint_t* LA, fint_t* IW, fint_t* LIW,
            double* W, fint_t* MAXFRT, double* RHS, fint_t* IW1, fint_t* NSTEPS,
 ...
 *	M a 2 7 S p a r s e S o l v e r
 */
Ma27SparseSolver::Ma27SparseSolver(const Ma27SparseSolver& rhs) { copy(rhs); }

/*
 *	~ M a 2 7 S p a r s e S o l v e r
 */
Ma27SparseSolver::~Ma27SparseSolver() { clear(); }

/*
 *	o p e r a t o r =
 */
Ma27SparseSolver& Ma27SparseSolver::operator=(const SparseSolver& rhs) {
  const Ma27SparseSolver* ma27_rhs =
      dynamic_cast<const Ma27SparseSolver*>(&rhs);
  if (!ma27_rhs) {
    fprintf(getGlobalMessageHandler()->getOutputFile(),
            "Error in Ma27SparseSolver& Ma27SparseSolver::operator=( const "
            "SparseSolver& rhs )\n");
 ...
 */
returnValue Ma27SparseSolver::setMatrixData(int_t dim_, int_t numNonzeros_,
...
returnValue Ma27SparseSolver::factorize() {
  ...
  MA27AD(&dim, &numNonzeros, irn_ma27, jcn_ma27, iw_ma27, &liw_ma27, ikeep_ma27,
         iw1_ma27, &nsteps_ma27, &iflag_ma27, icntl_ma27, cntl_ma27, info_ma27,
         &ops_ma27);
  ...
  MA27BD(&dim, &numNonzeros, irn_ma27, jcn_ma27, a_ma27, &la_ma27, iw_ma27,
         &liw_ma27, ikeep_ma27, &nsteps_ma27, &maxfrt_ma27, iw1_ma27,
         icntl_ma27, cntl_ma27, info_ma27);
  ...
  } else if (iflag == -5) { /* DJ: I think this is more severe. Can this
                               actually happen? */
    rank = -1;
    return RET_KKT_MATRIX_SINGULAR;
  } else if (iflag != 0) {
 ...
 *	s o l v e
 */
returnValue Ma27SparseSolver::solve(int_t dim_, const real_t* const rhs,
                                    real_t* const sol) {
  /* consistency check */
  if (dim_ != dim)
  ...
  for (int_t i = 0; i < dim; ++i)
    sol[i] = rhs[i];
  MA27CD(&dim, a_ma27, &la_ma27, iw_ma27, &liw_ma27, w_ma27, &maxfrt_ma27, sol,
         iw1_ma27, &nsteps_ma27, icntl_ma27, cntl_ma27);
 ...
 *	g e t R a n k
 */
int_t Ma27SparseSolver::getRank() { return rank; }
 ...
 */
extern void MA57ID(double* cntl, fint_t* icntl);

/*
 *  MA57AD -- Symbolic Factorization.
 */
extern void
MA57AD(fint_t* n,         /* Order of matrix. */
 ...
 */
extern void
MA57BD(fint_t* n,      /* Order of matrix. */
       fint_t* ne,     /* Number of entries. */
 ...
 */
extern void
MA57CD(fint_t* job,    /* Solution job.  Solve for... */
                       /* JOB <= 1:  A */
                       /* JOB == 2:  PLP^t */
                       /* JOB == 3:  PDP^t */
                       /* JOB >= 4:  PL^t P^t */
...
Ma57SparseSolver::Ma57SparseSolver() : SparseSolver() {
  ...
  icntl_ma57[15] =
      1; /* Place small pivots at the end of the factorization (default: 0) */

  /* \todo good default values?
  cntl_ma57[1] = 5.0e-16;		// Pivots smaller than this are treated
  as zero and are placed at the end of the factorization (default: 1e-20)
  cntl_ma57[0] = 0.5;			// Set pivot tolerance: Higher values =
  more stable but slower/less sparse (default: 0.01, max 0.5) */
}

/*
 *	M a 5 7 S p a r s e S o l v e r
 */
Ma57SparseSolver::Ma57SparseSolver(const Ma57SparseSolver& rhs) { copy(rhs); }

/*
 *	~ M a 5 7 S p a r s e S o l v e r
 */
Ma57SparseSolver::~Ma57SparseSolver() { clear(); }

/*
 *	o p e r a t o r =
 */
Ma57SparseSolver& Ma57SparseSolver::operator=(const SparseSolver& rhs) {
  const Ma57SparseSolver* ma57_rhs =
      dynamic_cast<const Ma57SparseSolver*>(&rhs);
  if (!ma57_rhs) {
    fprintf(getGlobalMessageHandler()->getOutputFile(),
            "Error in Ma57SparseSolver& Ma57SparseSolver::operator=( const "
            "SparseSolver& rhs )\n");
    throw; /* TODO: More elegant exit? */
  }
  if (this != ma57_rhs) {
 ...
 *	s e t M a t r i x D a t a
 */
returnValue Ma57SparseSolver::setMatrixData(int_t dim_, int_t numNonzeros_,
                                            const int_t* const irn,
                                            const int_t* const jcn,
                                            const real_t* const avals) {
  reset();
...
returnValue Ma57SparseSolver::factorize() {
  ...
  MA57BD(&dim, &numNonzeros, a_ma57, fact_ma57, &lfact_ma57, ifact_ma57,
         &lifact_ma57, &lkeep_ma57, keep_ma57, iwork_ma57, icntl_ma57,
         cntl_ma57, info_ma57, rinfo_ma57);
  ...
  for (k = 0; k < nblk; k++) {
    ...
    for (kk = 0; kk < nrows; kk++)
      pivots[count++] =
          ifact_ma57[iwpos + 2 + kk] - 1; /* convert Fortran to C indices! */
 ...
 *	s o l v e
 */
returnValue Ma57SparseSolver::solve(int_t dim_, const real_t* const rhs,
                                    real_t* const sol) {
  /* consistency check */
  if (dim_ != dim)
    return THROWERROR(RET_INVALID_ARGUMENTS);
 ...
 *	g e t R a n k
 */
int_t Ma57SparseSolver::getRank() { return rank; }
...
returnValue DummySparseSolver::setMatrixData(
    int_t dim,                /**< Dimension of the linear system. */
...
returnValue DummySparseSolver::solve(
    int_t dim,               /**< Dimension of the linear system. */
