...
public:
  ...
  virtual returnValue setMatrixData(
  ...
  /** Compute factorization of current matrix.  This method must be called
   * before solve.*/
  virtual returnValue factorize() = 0;

  /** Solve linear system with most recently set matrix data. */
  virtual returnValue
  solve(int_t dim,               /**< Dimension of the linear system. */
...
/**
 *	\brief Implementation of the linear solver interface using Harwell's
 *MA27.
 *
 *	\author Andreas Waechter, Dennis Janka
 *	\version 3.2
...
public:
  ...
  /** Assignment operator (deep copy). */
  virtual Ma27SparseSolver&
  operator=(const SparseSolver& rhs /**< Rhs object. */
  );
...
private:
  ...
  double*
      a_ma27; /**< matrix/factor for MA27 (A in MA27).  If have_factorization is
                 false, it contains the matrix entries (and has length
                 numNonzeros), otherwise the factor (and has length la_ma27). */
  ...
  bool have_factorization; /**< flag indicating whether factorization for
                              current matrix has already been computed */
...
/**
 *	\brief Implementation of a dummy sparse solver. An error is thrown if a
