...
/** Macro verifying that two numerical values are equal in order to pass unit
 * test. */
...
/** Macro verifying that two numerical values are close to each other in order
 * to pass unit test. */
#define QPOASES_TEST_FOR_NEAR(x, y)                                            \
  if (REFER_NAMESPACE_QPOASES getAbs((x) - (y)) /                              \
          REFER_NAMESPACE_QPOASES getMax(1.0,                                  \
                                         REFER_NAMESPACE_QPOASES getAbs(x)) >= \
      1e-10) {                                                                 \
    return TEST_FAILED;                                                        \
  }

/** Macro verifying that first quantity is lower or equal than second one in
 * order to pass unit test. */
