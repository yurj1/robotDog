...
void dgemm_(const char*, const char*, const la_uint_t*, const la_uint_t*,
            const la_uint_t*, const double*, const double*, const la_uint_t*,
            const double*, const la_uint_t*, const double*, double*,
            const la_uint_t*);
/** Performs one of the matrix-matrix operation in single precision. */
void sgemm_(const char*, const char*, const la_uint_t*, const la_uint_t*,
            const la_uint_t*, const float*, const float*, const la_uint_t*,
            const float*, const la_uint_t*, const float*, float*,
            const la_uint_t*);
...
void dsyr2_(const char*, const la_uint_t*, const double*, const double*,
            const la_uint_t*, const double*, const la_uint_t*, double*,
            const la_uint_t*);
/** Performs a symmetric rank 2 operation in single precision. */
void ssyr2_(const char*, const la_uint_t*, const float*, const float*,
            const la_uint_t*, const float*, const la_uint_t*, float*,
            const la_uint_t*);

/** Calculates the Cholesky factorization of a real symmetric positive definite
 * matrix in double precision. */
void dpotrf_(const char*, const la_uint_t*, double*, const la_uint_t*,
             la_int_t*);
/** Calculates the Cholesky factorization of a real symmetric positive definite
 * matrix in single precision. */
void spotrf_(const char*, const la_uint_t*, float*, const la_uint_t*,
             la_int_t*);

/** Computes a QR factorization of a real M-by-N matrix A in double precision */
/* void dgeqrf_(	const la_uint_t* M, const la_uint_t* N, double* A, const
   la_uint_t* LDA,
                                        double* TAU, double* WORK, const
   la_uint_t* LWORK, int *INFO );*/
/** Computes a QR factorization of a real M-by-N matrix A in single precision */
/* void sgeqrf_(	const la_uint_t* M, const la_uint_t* N, float* A, const
   la_uint_t* LDA, float* TAU, float* WORK, const la_uint_t* LWORK, int *INFO
   );*/

/** Multiplies C with orthogonal matrix Q**T as returned by geqrf (double
 * precision) */
/* void dormqr_(	const char* SIDE, const char* TRANS, const la_uint_t* M,
   const la_uint_t* N, const la_uint_t* K, double* A, const la_uint_t* LDA,
   double* TAU, double* C, const la_uint_t* LDC,
                                        double* WORK, const la_uint_t* LWORK,
   int *INFO );*/
/** Multiplies C with orthogonal matrix Q**T as returned by geqrf (single
 * precision) */
/* void sormqr_(	const char* SIDE, const char* TRANS, const la_uint_t* M,
   const la_uint_t* N, const la_uint_t* K, float* A, const la_uint_t* LDA,
   float* TAU, float* C, const la_uint_t* LDC, float* WORK, const la_uint_t*
   LWORK, int *INFO );*/
...
/** Estimate the reciprocal of the condition number of a triangular matrix in
 * double precision */
void dtrcon_(const char* NORM, const char* UPLO, const char* DIAG,
             const la_uint_t* N, double* A, const la_uint_t* LDA, double* RCOND,
             double* WORK, const la_uint_t* IWORK, la_int_t* INFO);
/** Estimate the reciprocal of the condition number of a triangular matrix in
 * single precision */
void strcon_(const char* NORM, const char* UPLO, const char* DIAG,
             const la_uint_t* N, float* A, const la_uint_t* LDA, float* RCOND,
             float* WORK, const la_uint_t* IWORK, la_int_t* INFO);
}
