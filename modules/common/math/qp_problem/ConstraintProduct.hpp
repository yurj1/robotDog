 ...
 *	current iterate to speed-up QP solution in case of a specially
 *structured constraint matrix.
 */
...
/**
 *	\brief Interface for specifying user-defined evaluations of constraint
 *products.
 *
 *	A class which allows to specify a user-defined function for evaluating
 *the constraint product at the current iterate to speed-up QP solution in case
...
public:
  ...
  /** Assignment operator. */
  ConstraintProduct&
  operator=(const ConstraintProduct& toCopy /**< Rhs object. */
  ) {
    return *this;
  }
  ...
  virtual int_t operator()(
      int_t constrIndex,     /**< Number of constraint to be evaluated. */
