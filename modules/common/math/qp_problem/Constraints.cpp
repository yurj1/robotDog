 ...
 *	\date 2007-2017
 *
 *	Implementation of the Constraints class designed to manage working sets
 *of constraints within a QProblem.
 */
 ...
 *	C o n s t r a i n t s
 */
Constraints::Constraints(int_t _n) : SubjectTo(_n) { init(_n); }

/*
 *	C o n s t r a i n t s
 */
Constraints::Constraints(const Constraints& rhs) : SubjectTo(rhs) { copy(rhs); }
...
returnValue Constraints::setupConstraint(int_t number,
  ...
  switch (_status) {
  case ST_INACTIVE:
    if (this->addIndex(this->getInactive(), number, _status) !=
        SUCCESSFUL_RETURN)
 ...
 */
returnValue Constraints::setupAllInactive() { return setupAll(ST_INACTIVE); }

/*
 *	s e t u p A l l L o w e r
 */
returnValue Constraints::setupAllLower() { return setupAll(ST_LOWER); }

/*
 *	s e t u p A l l U p p e r
 */
returnValue Constraints::setupAllUpper() { return setupAll(ST_UPPER); }
...
returnValue Constraints::moveActiveToInactive(int_t number) {
  ...
  if (this->addIndex(this->getInactive(), number, ST_INACTIVE) !=
      SUCCESSFUL_RETURN)
    return THROWERROR(RET_MOVING_BOUND_FAILED);
...
returnValue Constraints::flipFixed(int_t number) {
  ...
  if (status != 0)
    ...
    case ST_LOWER:
      status[number] = ST_UPPER;
      break;
    case ST_UPPER:
      status[number] = ST_LOWER;
      break;
    default:
      return THROWERROR(RET_MOVING_CONSTRAINT_FAILED);
    }
...
returnValue Constraints::print() {
  ...
  snprintf(myPrintfString, MAX_STRING_LENGTH,
           "Constraints object comprising %d constraints (%d inactive, %d "
           "active):\n",
 ...
 *	c l e a r
 */
returnValue Constraints::clear() { return SUCCESSFUL_RETURN; }
