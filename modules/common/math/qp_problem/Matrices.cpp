...
returnValue Matrix::getSparseSubmatrix(
    const Indexlist* const irows, const Indexlist* const icols, int_t rowoffset,
    int_t coloffset, int_t& numNonzeros, int_t* irn, int_t* jcn, real_t* avals,
  ...
  return getSparseSubmatrix(irows->getLength(), rowsNumbers, icols->getLength(),
                            colsNumbers, rowoffset, coloffset, numNonzeros, irn,
                            jcn, avals, only_lower_triangular);
}

returnValue
Matrix::getSparseSubmatrix(const Indexlist* const irows, int_t idx_icol,
                           int_t rowoffset, int_t coloffset, int_t& numNonzeros,
                           int_t* irn, int_t* jcn, real_t* avals,
                           BooleanType only_lower_triangular) const {
  int_t* rowsNumbers;
  irows->getNumberArray(&rowsNumbers);

  return getSparseSubmatrix(irows->getLength(), rowsNumbers, 1, &idx_icol,
                            rowoffset, coloffset, numNonzeros, irn, jcn, avals,
                            only_lower_triangular);
}

returnValue
Matrix::getSparseSubmatrix(int_t idx_row, const Indexlist* const icols,
                           int_t rowoffset, int_t coloffset, int_t& numNonzeros,
                           int_t* irn, int_t* jcn, real_t* avals,
                           BooleanType only_lower_triangular) const {
  int_t* colsNumbers;
  ...
  return getSparseSubmatrix(1, &idx_row, icols->getLength(), colsNumbers,
                            rowoffset, coloffset, numNonzeros, irn, jcn, avals,
                            only_lower_triangular);
}
...
real_t DenseMatrix::diag(int_t i) const { return val[i * (leaDim + 1)]; }
...
BooleanType DenseMatrix::isDiag() const {
  ...
  for (i = 0; i < nRows; ++i)
    for (j = 0; j < i; ++j)
      if ((getAbs(val[i * leaDim + j]) > EPS) ||
          (getAbs(val[j * leaDim + i]) > EPS))
        return BT_FALSE;

  return BT_TRUE;
}

real_t DenseMatrix::getNorm(int_t type) const {
...
returnValue DenseMatrix::getRow(int_t rNum, const Indexlist* const icols,
                                real_t alpha, real_t* row) const {
  int_t i;
  if (icols != 0) {
...
returnValue DenseMatrix::getCol(int_t cNum, const Indexlist* const irows,
                                real_t alpha, real_t* col) const {
  int_t i;
...
returnValue DenseMatrix::getSparseSubmatrix(
    int_t irowsLength, const int_t* const irowsNumber, int_t icolsLength,
    const int_t* const icolsNumber, int_t rowoffset, int_t coloffset,
    int_t& numNonzeros, int_t* irn, int_t* jcn, real_t* avals,
...
returnValue DenseMatrix::times(int_t xN, real_t alpha, const real_t* x,
                               int_t xLD, real_t beta, real_t* y,
                               int_t yLD) const {
  ...
  GEMM("TRANS", "NOTRANS", &_nRows, &_xN, &_nCols, &alpha, val, &_leaDim, x,
       &_xLD, &beta, y, &_yLD);
  return SUCCESSFUL_RETURN;
}

returnValue DenseMatrix::transTimes(int_t xN, real_t alpha, const real_t* x,
                                    int_t xLD, real_t beta, real_t* y,
                                    int_t yLD) const {
  la_uint_t _xN = (la_uint_t)xN;
  la_uint_t _nRows = (la_uint_t)nRows;
  ...
  /* Call BLAS. Mind row major format! */
  GEMM("NOTRANS", "NOTRANS", &_nCols, &_xN, &_nRows, &alpha, val, &_leaDim, x,
       &_xLD, &beta, y, &_yLD);
  return SUCCESSFUL_RETURN;
}

returnValue DenseMatrix::times(const Indexlist* const irows,
                               const Indexlist* const icols, int_t xN,
                               real_t alpha, const real_t* x, int_t xLD,
                               real_t beta, real_t* y, int_t yLD,
    ...
    for (k = 0; k < xN; k++)
      for (j = 0; j < irows->length; j++) {
        ...
        for (i = 0; i < icols->length; i++) {
          col = icols->iSort[i];
          y[col + k * yLD] +=
              val[irows->number[row] * leaDim + icols->number[col]] *
              x[row + k * xLD];
        }
      }
  else if (isEqual(alpha, -1.0) == BT_TRUE)
    ...
    for (k = 0; k < xN; k++)
      for (j = 0; j < irows->length; j++) {
        ...
        for (i = 0; i < icols->length; i++) {
          col = icols->iSort[i];
          y[col + k * yLD] +=
              alpha * val[irows->number[row] * leaDim + icols->number[col]] *
              x[row + k * xLD];
        }
      }
...
Matrix* SymDenseMat::duplicate() const { return duplicateSym(); }
...
returnValue SymDenseMat::bilinear(const Indexlist* const icols, int_t xN,
                                  const real_t* x, int_t xLD, real_t* y,
                                  int_t yLD) const {
  int_t ii, jj, kk, col;
  int_t i, j, k, irA;
...
SparseMatrix::SparseMatrix()
    : nRows(0), nCols(0), ir(0), jc(0), jd(0), val(0) {}

SparseMatrix::SparseMatrix(int_t nr, int_t nc, sparse_int_t* r, sparse_int_t* c,
                           real_t* v)
    : nRows(nr), nCols(nc), ir(r), jc(c), jd(0), val(v) {
  doNotFreeMemory();
}
...
SparseMatrix::SparseMatrix(int_t nr, int_t nc, int_t ld, const real_t* const v)
  ...
  for (j = 0; j < nCols; j++) {
    jc[j] = nnz;
    for (i = 0; i < nRows; i++)
      if ((isZero(v[i * ld + j], 0.0) == BT_FALSE) ||
          (i == j)) /* also include zero diagonal elemets! */
      {
        ir[nnz] = i;
        val[nnz++] = v[i * ld + j];
      }
...
real_t SparseMatrix::getNorm(int_t type) const {
...
returnValue SparseMatrix::getRow(int_t rNum, const Indexlist* const icols,
                                 real_t alpha, real_t* row) const {
  long i, j, k;
    ...
    if (isEqual(alpha, 1.0) == BT_TRUE)
      ...
      for (k = 0; k < icols->length; k++) {
        ...
        for (i = jc[j]; i < jc[j + 1] && ir[i] < rNum; i++)
          ;
        row[icols->iSort[k]] =
            (i < jc[j + 1] && ir[i] == rNum) ? alpha * val[i] : 0.0;
      }
  } else {
    if (isEqual(alpha, 1.0) == BT_TRUE)
      for (j = 0; j < nCols; j++) {
...
returnValue SparseMatrix::getCol(int_t cNum, const Indexlist* const irows,
                                 real_t alpha, real_t* col) const {
  long i, j;
...
returnValue SparseMatrix::getSparseSubmatrix(
    int_t irowsLength, const int_t* const irowsNumber, int_t icolsLength,
    const int_t* const icolsNumber, int_t rowoffset, int_t coloffset,
    int_t& numNonzeros, int_t* irn, int_t* jcn, real_t* avals,
    BooleanType only_lower_triangular /*= BT_FALSE */) const {
...
returnValue SparseMatrix::times(int_t xN, real_t alpha, const real_t* x,
                                int_t xLD, real_t beta, real_t* y,
                                int_t yLD) const {
  long i, j, k;
...
returnValue SparseMatrix::transTimes(int_t xN, real_t alpha, const real_t* x,
                                     int_t xLD, real_t beta, real_t* y,
                                     int_t yLD) const {
  long i, j, k;
...
returnValue SparseMatrix::times(const Indexlist* const irows,
  ...
  // First, work with full, unordered copy of y and store matrix times x in
  // there
    ...
    if (xN == 1) {
      ...
      for (l = 0; l < icols->length; l++) {
        ...
        if (isZero(xmax) == BT_FALSE) {
          ...
          for (i = jc[j]; i < jc[j + 1]; i++)
            for (k = 0; k < xN; k++)
              // AW: Maybe it makes more sense to order ytmp by vectors, not
              // vector entries, for better cache peformance?
  ...
  if (yCompr == BT_TRUE) {
    if (isZero(beta) == BT_TRUE)
      ...
      for (k = 0; k < xN; k++)
        for (j = 0; j < irows->length; j++)
          y[j + k * yLD] =
              alpha * ytmp[irows->number[j] + k * yfullLength] - y[j + k * yLD];
    else if (isEqual(beta, 1.0) == BT_FALSE)
      for (k = 0; k < xN; k++)
        for (j = 0; j < irows->length; j++)
          y[j + k * yLD] = alpha * ytmp[irows->number[j] + k * yfullLength] +
    ...
    if (isZero(beta) == BT_TRUE)
      for (k = 0; k < xN; k++)
        for (j = 0; j < irows->length; j++)
          y[irows->number[j] + k * yLD] =
              alpha * ytmp[irows->number[j] + k * yfullLength];
    else if (isEqual(beta, 1.0) == BT_TRUE)
      for (k = 0; k < xN; k++)
        for (j = 0; j < irows->length; j++)
          y[irows->number[j] + k * yLD] =
...
SparseMatrixRow::SparseMatrixRow()
    : nRows(0), nCols(0), jr(0), ic(0), jd(0), val(0) {}

SparseMatrixRow::SparseMatrixRow(int_t nr, int_t nc, sparse_int_t* r,
                                 sparse_int_t* c, real_t* v)
    : nRows(nr), nCols(nc), jr(r), ic(c), jd(0), val(v) {
  doNotFreeMemory();
}

SparseMatrixRow::SparseMatrixRow(int_t nr, int_t nc, int_t ld,
...
real_t SparseMatrixRow::getNorm(int_t type) const {
...
returnValue SparseMatrixRow::getRow(int_t rNum, const Indexlist* const icols,
                                    real_t alpha, real_t* row) const {
  long i, j;
...
returnValue SparseMatrixRow::getCol(int_t cNum, const Indexlist* const irows,
                                    real_t alpha, real_t* col) const {
  long i, j, k, srt;
...
returnValue SparseMatrixRow::times(int_t xN, real_t alpha, const real_t* x,
                                   int_t xLD, real_t beta, real_t* y,
                                   int_t yLD) const {
  long i, j, k;
...
returnValue SparseMatrixRow::transTimes(int_t xN, real_t alpha, const real_t* x,
                                        int_t xLD, real_t beta, real_t* y,
                                        int_t yLD) const {
  long i, j, k;
...
returnValue SparseMatrixRow::writeToFile(FILE* output_file,
                                         const char* prefix) const {
  return THROWERROR(RET_NOT_YET_IMPLEMENTED);
}

Matrix* SymSparseMat::duplicate() const { return duplicateSym(); }
...
returnValue SymSparseMat::bilinear(const Indexlist* const icols, int_t xN,
                                   const real_t* x, int_t xLD, real_t* y,
                                   int_t yLD) const {
      ...
      if (ir[idx] == row) {
        ...
        if (row == col) /* diagonal element */
          ...
          for (i = 0; i < xN; i++)
            for (j = i; j < xN; j++)
              y[i * yLD + j] +=
                  val[idx] * (x[i * xLD + col] * x[j * xLD + row] +
                              x[i * xLD + row] * x[j * xLD + col]);
        idx++, k++;
      } else if (ir[idx] > row)
        k++;
      else
