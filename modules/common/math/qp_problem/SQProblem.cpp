 ...
 *	S Q P r o b l e m
 */
SQProblem::SQProblem(int_t _nV, int_t _nC, HessianType _hessianType,
 ...
 */
returnValue
SQProblem::hotstart(SymmetricMatrix* H_new, const real_t* const g_new,
                    Matrix* A_new, const real_t* const lb_new,
  ...
  if (setupNewAuxiliaryQP(H_new, A_new, lb_new, ub_new, lbA_new, ubA_new) !=
      SUCCESSFUL_RETURN)
  ...
  returnValue returnvalue =
      QProblem::hotstart(g_new, lb_new, ub_new, lbA_new, ubA_new, nWSR, cputime,
 ...
 */
returnValue
SQProblem::hotstart(const real_t* const H_new, const real_t* const g_new,
                    const real_t* const A_new, const real_t* const lb_new,
                    const real_t* const ub_new, const real_t* const lbA_new,
                    const real_t* const ubA_new, int_t& nWSR,
                    real_t* const cputime, const Bounds* const guessedBounds,
                    const Constraints* const guessedConstraints) {
 ...
 */
returnValue
SQProblem::hotstart(const char* const H_file, const char* const g_file,
 ...
 */
returnValue
SQProblem::hotstart(const real_t* const g_new, const real_t* const lb_new,
                    const real_t* const ub_new, const real_t* const lbA_new,
                    const real_t* const ubA_new, int_t& nWSR,
                    real_t* const cputime, const Bounds* const guessedBounds,
                    const Constraints* const guessedConstraints) {
  /* Call to hotstart function for fixed QP matrices. */
  return QProblem::hotstart(g_new, lb_new, ub_new, lbA_new, ubA_new, nWSR,
 ...
 */
returnValue
SQProblem::hotstart(const char* const g_file, const char* const lb_file,
                    const char* const ub_file, const char* const lbA_file,
                    const char* const ubA_file, int_t& nWSR,
                    real_t* const cputime, const Bounds* const guessedBounds,
                    const Constraints* const guessedConstraints) {
  /* Call to hotstart function for fixed QP matrices. */
  return QProblem::hotstart(g_file, lb_file, ub_file, lbA_file, ubA_file, nWSR,
                            cputime, guessedBounds, guessedConstraints);
}
...
returnValue SQProblem::setupNewAuxiliaryQP(SymmetricMatrix* H_new,
                                           Matrix* A_new, const real_t* lb_new,
                                           const real_t* ub_new,
  ...
  if (H_new != 0) {
    ...
    /* if no Hessian is specified, keep previous trivial Hessian (HST_ZERO or
       HST_IDENTITY), otherwise abort */
    if (H != 0)
  ...
  for (n_try = 0; n_try < 2; ++n_try) {
    ...
    for (int_t ii = 0; ii < nC; ++ii) {
      if (oldConstraints.getType(ii) == ST_EQUALITY &&
          constraints.getType(ii) == ST_BOUNDED) {
 ...
 *	s e t u p N e w A u x i l i a r y Q P
 */
returnValue SQProblem::setupNewAuxiliaryQP(
    const real_t* const H_new, const real_t* const A_new, const real_t* lb_new,
    const real_t* ub_new, const real_t* lbA_new, const real_t* ubA_new) {
  int_t nV = getNV();
  int_t nC = getNC();
  ...
  returnValue returnvalue =
      setupNewAuxiliaryQP(sH, dA, lb_new, ub_new, lbA_new, ubA_new);
