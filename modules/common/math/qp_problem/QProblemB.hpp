 ...
 *programming for problems with (simple) bounds only.
 */

#ifndef QPOASES_QPROBLEMB_HPP
#define QPOASES_QPROBLEMB_HPP

#include "modules/common/math/qp_problem/Flipper.hpp"
#include "modules/common/math/qp_problem/Options.hpp"
...
/**
 *	\brief Implements the online active set strategy for box-constrained
 *QPs.
 *
 *	Class for setting up and solving quadratic programs with bounds (= box
 *constraints) only. The main feature is the possibily to use the newly
...
public:
  ...
  /** Initialises a simply bounded QP problem with given QP data and tries to
   solve it *	using at most nWSR iterations. Depending on the parameter
   constellation it: \n *	1. 0,    0,    0 : starts with xOpt = 0, yOpt =
   0 and gB empty (or all implicit equality bounds), \n *	2. xOpt, 0, 0 :
   starts with xOpt, yOpt = 0 and obtain gB by "clipping", \n *	3. 0,    yOpt,
   0 : starts with xOpt = 0, yOpt and obtain gB from yOpt != 0, \n *	4. 0, 0,
   gB: starts with xOpt = 0, yOpt = 0 and gB, \n *	5. xOpt, yOpt, 0 :
   starts with xOpt, yOpt and obtain gB from yOpt != 0, \n *	6. xOpt, 0, gB:
   starts with xOpt, yOpt = 0 and gB, \n *	7. xOpt, yOpt, gB: starts with
   xOpt, yOpt and gB (assume them to be consistent!)
   *
  ...
  returnValue init(
      ...
      const real_t* const xOpt =
          0, /**< Optimal primal solution vector. A NULL pointer can be passed.
                \n (If a null pointer is passed, the old primal solution is
                kept!) */
      const real_t* const yOpt =
          0, /**< Optimal dual solution vector. A NULL pointer can be passed. \n
  ...
  /** Initialises a simply bounded QP problem with given QP data to be read from
   files and solves it *	using at most nWSR iterations. Depending on the
   parameter constellation it: \n *	1. 0,    0,    0 : starts with xOpt = 0,
   yOpt = 0 and gB empty (or all implicit equality bounds), \n
   *	2. xOpt, 0,    0 : starts with xOpt, yOpt = 0 and obtain gB by
   "clipping", \n *	3. 0,    yOpt, 0 : starts with xOpt = 0, yOpt and obtain
   gB from yOpt != 0, \n *	4. 0,    0,    gB: starts with xOpt = 0, yOpt =
   0 and gB, \n *	5. xOpt, yOpt, 0 : starts with xOpt, yOpt and obtain gB
   from yOpt != 0, \n *	6. xOpt, 0,    gB: starts with xOpt, yOpt = 0 and gB, \n
   *	7. xOpt, yOpt, gB: starts with xOpt, yOpt and gB (assume them to be
   consistent!)
   *
   *  Note: This function internally calls solveInitialQP for initialisation!
   *
  ...
  init(const char* const H_file, /**< Name of file where Hessian matrix is
       ...
       const real_t* const yOpt =
           0, /**< Optimal dual solution vector. A NULL pointer can be passed.
                 \n (If a null pointer is passed, the old dual solution is
                 kept!) */
       const Bounds* const guessedBounds =
           0, /**< Optimal working set of bounds for solution (xOpt,yOpt). \n
                                                               (If a null
  ...
  returnValue hotstart(
      ...
      const Bounds* const guessedBounds =
          0 /**< Optimal working set of bounds for solution (xOpt,yOpt). \n
                                                             (If a null pointer
               is passed, the previous working set is kept!) */
  );
...
protected:
  ...
  /** Determines type of new constraints and bounds (i.e. implicitly fixed,
   unbounded etc.). *	\return SUCCESSFUL_RETURN \n
                          RET_SETUPSUBJECTTOTYPE_FAILED */
  virtual returnValue
  setupSubjectToType(const real_t* const lb_new, /**< New lower bounds. */
  ...
  returnValue obtainAuxiliaryWorkingSet(
      ...
      Bounds*
          auxiliaryBounds /**< Input: Allocated bound object. \n
  ...
  returnValue
  areBoundsConsistent(const real_t* const lb, /**< Vector of lower bounds*/
  ...
  virtual returnValue
  backsolveR(const real_t* const b,  /**< Right hand side vector. */
  ...
  backsolveR(const real_t* const b,     /**< Right hand side vector. */
             BooleanType transposed,    /**< Indicates if the transposed system
                                           shall be solved. */
             BooleanType removingBound, /**< Indicates if function is called
                                           from "removeBound()". */
             real_t* const a            /**< Output: Solution vector */
             ) const;
  ...
  /** Sets internal infeasibility flag and throws given error in case the far
   bound *	strategy is not enabled (as QP might actually not be infeasible
   in this case). *	\return RET_HOTSTART_STOPPED_INFEASIBILITY \n
                          RET_ENSURELI_FAILED_CYCLING \n
                          RET_ENSURELI_FAILED_NOINDEX */
  returnValue setInfeasibilityFlag(
      returnValue returnvalue, /**< Returnvalue to be tunneled. */
      BooleanType doThrowError =
          BT_FALSE /**< Flag forcing to throw an error. */
  );

  /** Determines if next QP iteration can be performed within given CPU time
   limit. *	\return BT_TRUE: CPU time limit is exceeded, stop QP solution.
   \n BT_FALSE: Sufficient CPU time for next QP iteration. */
  BooleanType isCPUtimeLimitExceeded(
      const real_t* const
   ...
   *	\return SUCCESSFUL_RETURN */
  inline returnValue setH(SymmetricMatrix* H_new /**< New Hessian matrix (a
   ...
   *	\return SUCCESSFUL_RETURN */
  inline returnValue
  setH(const real_t* const H_new /**< New dense Hessian matrix (with correct
  ...
  inline returnValue
  setLB(int_t number, /**< Number of entry to be changed. */
   ...
   *			RET_INDEX_OUT_OF_BOUNDS */
  inline returnValue
  setUB(int_t number, /**< Number of entry to be changed. */
   ...
   *	\return SUCCESSFUL_RETURN */
  inline void
  computeGivens(real_t xold,  /**< Matrix entry to be normalised. */
  ...
  returnValue performRatioTest(
      ...
      const SubjectTo* const
          subjectTo, /**< Bound/Constraint object corresponding to ratios
                        to be checked. */
  ...
  /** Checks whether given ratio is blocking, i.e. limits the maximum step
   *length along the homotopy path to a value lower than given one. \return
   *SUCCESSFUL_RETURN */
  inline BooleanType
  isBlocking(real_t num,    /**< Numerator for performing the ratio test. */
             real_t den,    /**< Denominator for performing the ratio test. */
             real_t epsNum, /**< Numerator tolerance. */
             real_t epsDen, /**< Denominator tolerance. */
             real_t& t      /**< Input: Current maximum step length along the
                             * homotopy path,      Output: Updated maximum possible
                             * step      length along the homotopy path. */
             ) const;
   ...
   */
  SymSparseMat* createDiagSparseMat(
      int_t n,             /**< Row/column dimension of matrix to be created. */
...
private:
  ...
  returnValue setupAuxiliaryQPbounds(
      BooleanType useRelaxation /**< Flag indicating if inactive bounds shall be
                                   relaxed. */
  );
...
protected:
  /** Updates QP vectors, working sets and internal data structures in order to
   ...
   working set for bounds *	\return SUCCESSFUL_RETURN \n *
   RET_SETUP_AUXILIARYQP_FAILED */
...
private:
  ...
  BooleanType shallRefactorise(
      const Bounds* const guessedBounds /**< Guessed new working set. */
      ) const;
   ...
   */
protected:
  BooleanType freeHessian; /**< Flag indicating whether the Hessian matrix needs
                              to be de-allocated. */
  ...
  BooleanType haveCholesky; /**< Flag indicating whether Cholesky decomposition
  ...
  real_t regVal; /**< Holds the offset used to regularise Hessian matrix (zero
                    by default). */
  ...
  Options options; /**< Struct containing all user-defined options for solving
                      QPs. */

  Flipper flipper; /**< Struct for making a temporary copy of the matrix
                      factorisations. */

  TabularOutput tabularOutput; /**< Struct storing information for tabular
                                  output (printLevel == PL_TABULAR). */
};

END_NAMESPACE_QPOASES

#include "modules/common/math/qp_problem/QProblemB.ipp"
