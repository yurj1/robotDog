...
#include "modules/common/math/qp_problem/Bounds.hpp"
#include "modules/common/math/qp_problem/Constraints.hpp"

BEGIN_NAMESPACE_QPOASES

/**
 *	\brief Auxiliary class for storing a copy of the current matrix
 *factorisations.
 *
 *	This auxiliary class stores a copy of the current matrix factorisations.
 *It is used by the classe QProblemB and QProblem in case flipping bounds are
 *enabled.
 *
 *	\author Hans Joachim Ferreau, Andreas Potschka, Christian Kirches
 *	\version 3.2
...
public:
  ...
  /** Copies current values to non-null arguments (assumed to be allocated with
   *consistent size). \return SUCCESSFUL_RETURN */
  returnValue
  get(Bounds* const _bounds,               /**< Pointer to new bounds. */
