...
#include "modules/common/math/qp_problem/extras/OQPinterface.hpp"
 ...
 */
returnValue readOqpDimensions(const char* path, int_t& nQP, int_t& nV,
                              int_t& nC, int_t& nEC) {
 ...
 */
returnValue readOqpData(const char* path, int_t& nQP, int_t& nV, int_t& nC,
                        int_t& nEC, real_t** H, real_t** g, real_t** A,
...
returnValue solveOqpBenchmark(int_t nQP, int_t nV, int_t nC, int_t nEC,
                              ...
                              const real_t* const _A, const real_t* const lb,
                              const real_t* const ub, const real_t* const lbA,
                              const real_t* const ubA, BooleanType isSparse,
                              const Options& options, int_t& nWSR,
                              real_t& maxCPUtime, real_t& maxStationarity,
  ...
  returnValue returnvalue = solveOqpBenchmark(
      nQP, nV, nC, nEC, _H, g, _A, lb, ub, lbA, ubA, isSparse, BT_TRUE, options,
      nWSR, maxNWSR, avgNWSR, maxCPUtime, avgCPUtime, maxStationarity,
      maxFeasibility, maxComplementarity);
  nWSR = (int_t)maxNWSR;
 ...
 *	s o l v e O q p B e n c h m a r k
 */
returnValue solveOqpBenchmark(
    int_t nQP, int_t nV, int_t nC, int_t nEC, const real_t* const _H,
    const real_t* const g, const real_t* const _A, const real_t* const lb,
    const real_t* const ub, const real_t* const lbA, const real_t* const ubA,
    BooleanType isSparse, BooleanType useHotstarts, const Options& options,
    int_t maxAllowedNWSR, real_t& maxNWSR, real_t& avgNWSR, real_t& maxCPUtime,
    real_t& avgCPUtime, real_t& maxStationarity, real_t& maxFeasibility,
    real_t& maxComplementarity) {
  int_t k;
  ...
  for (k = 0; k < nQP; ++k) {
    ...
    if ((k == 0) || (useHotstarts == BT_FALSE)) {
      /* initialise */
      returnvalue = qp.init(H, gCur, A, lbCur, ubCur, lbACur, ubACur, nWSRcur,
                            &CPUtimeCur);
      ...
      /* hotstart */
      returnvalue =
          qp.hotstart(gCur, lbCur, ubCur, lbACur, ubACur, nWSRcur, &CPUtimeCur);
      if ((returnvalue != SUCCESSFUL_RETURN) &&
          (returnvalue != RET_MAX_NWSR_REACHED)) {
        delete A;
        delete H;
    ...
    getKktViolation(nV, nC, _H, gCur, _A, lbCur, ubCur, lbACur, ubACur, x, y,
                    stat, feas, cmpl);

    /* 6) Update maximum and average values. */
    if (((double)nWSRcur) > maxNWSR)
      maxNWSR = ((double)nWSRcur);
    if (stat > maxStationarity)
      maxStationarity = stat;
    if (feas > maxFeasibility)
      maxFeasibility = feas;
    if (cmpl > maxComplementarity)
      maxComplementarity = cmpl;
...
returnValue solveOqpBenchmark(int_t nQP, int_t nV, const real_t* const _H,
                              ...
                              const real_t* const ub, BooleanType isSparse,
                              const Options& options, int_t& nWSR,
                              real_t& maxCPUtime, real_t& maxStationarity,
                              real_t& maxFeasibility,
                              real_t& maxComplementarity) {
  ...
  returnValue returnvalue =
      solveOqpBenchmark(nQP, nV, _H, g, lb, ub, isSparse, BT_TRUE, options,
                        nWSR, maxNWSR, avgNWSR, maxCPUtime, avgCPUtime,
...
returnValue solveOqpBenchmark(int_t nQP, int_t nV, const real_t* const _H,
                              const real_t* const g, const real_t* const lb,
                              const real_t* const ub, BooleanType isSparse,
                              BooleanType useHotstarts, const Options& options,
                              int_t maxAllowedNWSR, real_t& maxNWSR,
                              real_t& avgNWSR, real_t& maxCPUtime,
                              real_t& avgCPUtime, real_t& maxStationarity,
                              real_t& maxFeasibility,
                              real_t& maxComplementarity) {
  int_t k;
 ...
 */
returnValue runOqpBenchmark(const char* path, BooleanType isSparse,
                            const Options& options, int_t& nWSR,
                            real_t& maxCPUtime, real_t& maxStationarity,
                            real_t& maxFeasibility,
                            real_t& maxComplementarity) {
  ...
  returnValue returnvalue = runOqpBenchmark(
      path, isSparse, BT_TRUE, options, nWSR, maxNWSR, avgNWSR, maxCPUtime,
      avgCPUtime, maxStationarity, maxFeasibility, maxComplementarity);
  nWSR = (int_t)maxNWSR;
...
returnValue runOqpBenchmark(const char* path, BooleanType isSparse,
  ...
  if (readOqpData(path, nQP, nV, nC, nEC, &H, &g, &A, &lb, &ub, &lbA, &ubA, 0,
  ...
  if (nC > 0) {
    returnvalue = solveOqpBenchmark(
        nQP, nV, nC, nEC, H, g, A, lb, ub, lbA, ubA, isSparse, useHotstarts,
        options, maxAllowedNWSR, maxNWSR, avgNWSR, maxCPUtime, avgCPUtime,
        maxStationarity, maxFeasibility, maxComplementarity);
    ...
    returnvalue = solveOqpBenchmark(
        nQP, nV, H, g, lb, ub, isSparse, useHotstarts, options, maxAllowedNWSR,
        maxNWSR, avgNWSR, maxCPUtime, avgCPUtime, maxStationarity,
        maxFeasibility, maxComplementarity);
