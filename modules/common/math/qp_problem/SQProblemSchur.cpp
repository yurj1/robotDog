 ...
 *	\author Andreas Waechter and Dennis Janka, based on QProblem.cpp by Hans
 *Joachim Ferreau, Andreas Potschka, Christian Kirches \version 3.2 \date
 *2012-2017
 *
 *	Implementation of the SQProblemSchur class which is able to use the
 *newly developed online active set strategy for parametric quadratic
 *programming. This implementation uses a Schur complement approach to solve the
 *linear systems.
 */

#include "modules/common/math/qp_problem/SQProblemSchur.hpp"
 ...
 *	Q P r o b l e m
 */
SQProblemSchur::SQProblemSchur(int_t _nV, int_t _nC, HessianType _hessianType,
                               int_t maxSchurUpdates)
  ...
  *sparseSolver = *(rhs.sparseSolver);
  ...
  if (nSmax > 0) {
    ...
    if (nS > 0) {
      ...
      memcpy(schurUpdateIndex, rhs.schurUpdateIndex,
             ((unsigned int)nS) * sizeof(int_t));
      memcpy(schurUpdate, rhs.schurUpdate,
             ((unsigned int)nS) * sizeof(SchurUpdateType));
    }
 ...
 */
returnValue
SQProblemSchur::setupAuxiliaryQP(SymmetricMatrix* H_new, Matrix* A_new,
                                 const real_t* lb_new, const real_t* ub_new,
                                 const real_t* lbA_new, const real_t* ubA_new) {
  int_t i;
  int_t nV = getNV();
  int_t nC = getNC();
  ...
  /* 1) Check if current active set is linearly independent and has the correct
   * inertia */
  ...
  if (returnvalue == SUCCESSFUL_RETURN && neig == getNAC()) {
     ...
     *    leaves no zero curvature exposed in the nullspace and can be used to
     * start QP solve. */
    if (options.printLevel == PL_HIGH)
      MyPrintf("In hotstart for new matrices, old working set is linearly "
               "independent and has correct inertia.\n");

    status = QPS_AUXILIARYQPSOLVED;
    return SUCCESSFUL_RETURN;
  } else if (returnvalue == SUCCESSFUL_RETURN && neig > getNAC()) {
    /* b) KKT matrix has too many negative eigenvalues. Try to correct the
     * inertia by adding bounds (reduce nullspace dimension). */
    if (options.printLevel == PL_HIGH)
      MyPrintf(
          "WARNING: In hotstart for new matrices, reduced Hessian for initial "
          "working set has %i negative eigenvalues, should be %i.\n",
          neig, getNAC());
  ...
  /* 2) If inertia correction has failed or factorization yielded some other
   * error, try to rebuild the active set with all simple bounds set according
   * to initialStatusBounds (Note: in exact arithmetic, this cannot happen) */
  if (options.printLevel == PL_HIGH)
    MyPrintf("WARNING: hotstart for old active set failed. Trying to rebuild a "
             "working set.\n");
  ...
  for (i = 0; i < nV; i++) {
...
#endif
    {
      if (oldBounds.getStatus(i) == ST_INACTIVE)
        if (oldBounds.setStatus(i, options.initialStatusBounds) !=
            SUCCESSFUL_RETURN)
          return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
    }
  }
...
/* Set all equalities active */
  ...
  if (setupAuxiliaryWorkingSet(&oldBounds, &oldConstraints, BT_TRUE) !=
      SUCCESSFUL_RETURN)
    return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
 ...
 */
returnValue SQProblemSchur::setupAuxiliaryWorkingSet(
   ...
   *       inactive or active at the other bound according to
  ...
  for (i = 0; i < nV; ++i) {
    if ((bounds.getStatus(i) == ST_INACTIVE) &&
      ...
      if (bounds.moveFreeToFixed(i, auxiliaryBounds->getStatus(i)) !=
          SUCCESSFUL_RETURN)
        return THROWERROR(RET_SETUP_WORKINGSET_FAILED);
  }
  ...
  for (i = 0; i < nC; ++i) {
    if ((constraints.getStatus(i) == ST_INACTIVE) &&
      ...
      if (constraints.moveInactiveToActive(
              i, auxiliaryConstraints->getStatus(i)) != SUCCESSFUL_RETURN)
        return THROWERROR(RET_SETUP_WORKINGSET_FAILED);
  }
  ...
  /* III.2.) Check if inertia is correct. If so, we now have a linearly
   * independent working set with a pos def reduced Hessian */
  int_t neig = sparseSolver->getNegativeEigenvalues();
  if (neig == getNAC()) {
    /* We now have a linearly independent working set with a pos def reduced
     * Hessian. We need to correct the QP bounds and gradient after this. */
    return SUCCESSFUL_RETURN;
  }

  /* IV.) INERTIA CORRECTION IF NECESSARY */

  /* We now have a fresh factorization and can start the usual inertia
   * correction routine */
  if (options.printLevel == PL_HIGH)
    MyPrintf("WARNING: In setupAuxiliaryWorkingSet: Initial working set "
             "reduced Hessian has %i negative eigenvalues, should be %i.\n",
             neig, getNAC());
 ...
 */
returnValue SQProblemSchur::setupTQfactorisation() { return SUCCESSFUL_RETURN; }
    ...
    if (nS < 0 || nS == nSmax) {
      ...
      if (retval != SUCCESSFUL_RETURN) {
        if (retval == RET_KKT_MATRIX_SINGULAR && options.printLevel == PL_HIGH)
          MyPrintf("In addConstraint: KKT matrix singular when resetting Schur "
                   "complement\n");
        else if (options.printLevel == PL_HIGH)
          MyPrintf("In addConstraint, resetSchurComplement failed with retval "
                   "= %d\n",
                   retval);
        return THROWERROR(RET_ADDCONSTRAINT_FAILED);
      }
      ...
      sparse_int_t* MNpos =
          new sparse_int_t[nFRStart + nS]; // This is an overestimate
      ...
      if (constraintProduct != 0) {
        MyPrintf("In SQProblemSchur::addConstraint, constraintProduct not yet "
                 "implemented.\n");
        return THROWERROR(RET_NOT_YET_IMPLEMENTED);
      }
      int_t numNonzerosA;
      A->getSparseSubmatrix(1, &number, icolsLength, icolsNumber, 0, 0,
                            numNonzerosA, irn, jcn, vals);
      delete[] irn;
      ...
      returnValue returnvalue = addToSchurComplement(
          number, SUT_ConAdded, numNonzerosM, MNpos, MNvals, numNonzerosN,
          MNpos + nFRStart, MNvals + nFRStart, 0.0);
...
returnValue SQProblemSchur::addConstraint_checkLISchur(int_t number,
  ...
  /* For the Schur complement version we only use options.enableFullLITests =
   * TRUE */
  {
    /*
...
returnValue SQProblemSchur::addConstraint_ensureLI(int_t number,
  ...
  /* 1) Coefficients of linear combination, have already been computed, but we
   * need to correct the sign.  */
  int_t i, ii;
  ...
  if (y_min_number >= 0) {
    ...
    if (y_min_isBound == BT_TRUE) {
      ...
      getGlobalMessageHandler()->throwInfo(RET_REMOVE_FROM_ACTIVESET,
                                           messageString, __FUNCTION__,
                                           __FILE__, __LINE__, VS_VISIBLE);
#endif
...
farewell:
  ...
  getGlobalMessageHandler()->throwInfo(RET_LI_RESOLVED, 0, __FUNCTION__,
                                       __FILE__, __LINE__, VS_VISIBLE);
...
returnValue SQProblemSchur::addBound(int_t number, SubjectToStatus B_status,
  ...
  if (!found) {
    if (nS < 0 || nS == nSmax) {
      ...
      if (retval != SUCCESSFUL_RETURN) {
        if (retval == RET_KKT_MATRIX_SINGULAR && options.printLevel == PL_HIGH)
          MyPrintf("In addBound: KKT matrix singular when resetting Schur "
                   "complement\n");
        else if (options.printLevel == PL_HIGH)
          MyPrintf(
      ...
      for (int_t i = 0; i < nFRStart; i++)
        if (FR_idxStart[i] == number) {
          real_t one = 1.0;
          sparse_int_t pos = i;
          if (addToSchurComplement(number, SUT_VarFixed, 1, &pos, &one, 0, 0, 0,
                                   0.0) != SUCCESSFUL_RETURN)
            return THROWERROR(RET_ADDBOUND_FAILED);
          found = true;
          break;
        }
 ...
 */
returnValue SQProblemSchur::addBound_checkLISchur(int_t number, real_t* xiC,
                                                  real_t* xiB) {
  returnValue returnvalue = RET_LINEARLY_DEPENDENT;
...
returnValue SQProblemSchur::removeConstraint(int_t number,
  ...
  if (!found) {
    if (nS < 0 || nS == nSmax) {
      ...
      if (retval != SUCCESSFUL_RETURN) {
        if (retval == RET_KKT_MATRIX_SINGULAR && options.printLevel == PL_HIGH)
          MyPrintf("In removeConstraint: KKT matrix singular when resetting "
                   "Schur complement\n");
        else if (options.printLevel == PL_HIGH)
          MyPrintf("In removeConstraint, resetSchurComplement failed with "
                   "retval = %d\n",
                   retval);
        return THROWERROR(RET_REMOVECONSTRAINT_FAILED);
      }
      ...
      for (int_t i = 0; i < nACStart; i++)
        if (AC_idxStart[i] == number) {
          ...
          if (addToSchurComplement(number, SUT_ConRemoved, 1, &pos, &one, 0, 0,
                                   0, 0.0) != SUCCESSFUL_RETURN)
            return THROWERROR(RET_REMOVECONSTRAINT_FAILED);
          found = true;
          break;
        }
  ...
  /* Now we have a new Schur complement (might be smaller, larger, or empty).
  ...
  if ((options.enableFlippingBounds == BT_TRUE) && (allowFlipping == BT_TRUE) &&
    ...
    if (sModType == 1) { /* Case 1: We added a row and column to S. */
      ...
      /* Check if a direction of negative curvature showed up, i.e. determinants
       * have THE SAME sign */
      ...
      } else { /* Determinants have the correct sign, compute QR of new (larger)
                  S */
        updateSchurQR(idxDeleted);
      }
      ...
      } else { /* Determinants have the correct sign, compute QR of new
                  (smaller) S */
        updateSchurQR(idxDeleted);
      }
    } else if (sModType == 3) { /* Case 3: S was reset. */

      /* Check inertia of new factorization given by the sparse solver: must be
       * ( nFR, nAC, 0 ) */
      int_t neig = sparseSolver->getNegativeEigenvalues();
  ...
  if (rcondS < options.rcondSMin) {
    ...
    if (retval != SUCCESSFUL_RETURN) {
      if (retval == RET_KKT_MATRIX_SINGULAR && options.printLevel == PL_HIGH)
        ...
        MyPrintf("In removeConstraint, resetSchurComplement failed with retval "
                 "= %d\n",
                 retval);
      return THROWERROR(RET_ADDCONSTRAINT_FAILED);
    }
...
returnValue SQProblemSchur::removeBound(int_t number,
  ...
  if (!found) {
    if (nS < 0 || nS == nSmax) {
      ...
      if (retval != SUCCESSFUL_RETURN) {
        if (retval == RET_KKT_MATRIX_SINGULAR && options.printLevel == PL_HIGH)
          MyPrintf("In removeBound: KKT matrix singular when resetting Schur "
                   "complement\n");
        else if (options.printLevel == PL_HIGH)
          MyPrintf(
      ...
      int_t numNonzerosM = 0;
      sparse_int_t* Mpos =
          new sparse_int_t[nFRStart + nACStart + nS]; // This is an overestimate
      ...
      default:
        ...
        H->getSparseSubmatrix(iLength, iNumber, 1, &number, 0, 0, numNonzeros,
                              irn, jcn, vals);
      ...
      if (constraintProduct != 0) {
        MyPrintf("In SQProblemSchur::removeBound, constraintProduct not yet "
                 "implemented.\n");
        return THROWERROR(RET_NOT_YET_IMPLEMENTED);
      }
      ...
      A->getSparseSubmatrix(iLength, iNumber, 1, &number, 0, 0, numNonzeros,
                            irn, jcn, vals);
      ...
      returnvalue =
          addToSchurComplement(number, SUT_VarFreed, numNonzerosM, Mpos, Mvals,
                               numNonzerosN, Npos, Nvals, N_diag);
...
real_t SQProblemSchur::calcDetSchur(int_t idxDel) {
  ...
  /* Case 2: row and column idxDel have been deleted from S */
    ...
    for (i = idxDel; i < nS; i++) {
      computeGivens(tempR[i + i * dim], tempR[(i + 1) + i * dim],
                    tempR[i + i * dim], tempR[(i + 1) + i * dim], c, s);
      nu = s / (1.0 + c);
      /// \todo I think we do not need to transform all columns of R, i+3 or so
      /// should be sufficient
      for (j = i + 1; j < nS; j++)
        applyGivens(c, s, nu, tempR[i + j * dim], tempR[(i + 1) + j * dim],
                    tempR[i + j * dim], tempR[(i + 1) + j * dim]);

      /* Simultaneously transform relevant column of Q**T */
      applyGivens(c, s, nu, tempColQ[i], tempColQ[i + 1], tempColQ[i],
                  tempColQ[i + 1]);
    }

    /* Delete row: nS Givens rotations to transform last column (and row!) of
     * (old) Q**T to getAbs((nS+1)-th unity vector) */
    for (i = nS; i > 0; i--) {
      computeGivens(tempColQ[nS], tempColQ[i - 1], tempColQ[nS],
                    tempColQ[i - 1], c, s);
      nu = s / (1.0 + c);

      /* Simultaneously transform diagonal elements of R (coldim is already one
       * less than Q) */
      applyGivens(c, s, nu, tempR[nS + (i - 1) * dim],
                  tempR[(i - 1) + (i - 1) * dim], tempR[nS + (i - 1) * dim],
                  tempR[(i - 1) + (i - 1) * dim]);
    }

    /* Note that we implicitly did a row permutation of Q.
     * If we did an  odd permutation AND deleted a positive unity vector or
     * if we did an even permutation AND deleted a negative unity vector, then
     * det(Q)=-1
...
returnValue SQProblemSchur::updateSchurQR(int_t idxDel) {
  ...
  if (idxDel < 0) {
    ...
    for (i = 0; i < nS - 1; i++) {
      computeGivens(R_[i + i * nSmax], R_[(nS - 1) + i * nSmax],
                    R_[i + i * nSmax], R_[(nS - 1) + i * nSmax], c, s);
      nu = s / (1.0 + c);
      for (j = i + 1; j < nS; j++)
        applyGivens(c, s, nu, R_[i + j * nSmax], R_[(nS - 1) + j * nSmax],
                    R_[i + j * nSmax], R_[(nS - 1) + j * nSmax]);

      /* Simultaneously transform Q**T */
      for (j = 0; j < nS; j++)
        applyGivens(c, s, nu, Q_[j + i * nSmax], Q_[j + (nS - 1) * nSmax],
                    Q_[j + i * nSmax], Q_[j + (nS - 1) * nSmax]);
    }
  }
  /* Case 2: row and column idxDel have been deleted from S */
  else {
    ...
    for (i = idxDel; i < nS; i++) {
      computeGivens(R_[i + i * nSmax], R_[(i + 1) + i * nSmax],
                    R_[i + i * nSmax], R_[(i + 1) + i * nSmax], c, s);
      nu = s / (1.0 + c);
      for (j = i + 1; j < nS; j++)
        applyGivens(c, s, nu, R_[i + j * nSmax], R_[(i + 1) + j * nSmax],
                    R_[i + j * nSmax], R_[(i + 1) + j * nSmax]);

      /* Simultaneously transform (old) Q**T (coldim is one larger)*/
      for (j = 0; j < nS + 1; j++)
        applyGivens(c, s, nu, Q_[j + i * nSmax], Q_[j + (i + 1) * nSmax],
                    Q_[j + i * nSmax], Q_[j + (i + 1) * nSmax]);
    }
    ...
    /* IV: Delete row: nS Givens rotations to transform last column (and row!)
     * of (old) Q**T to getAbs((nS+1)-th unity vector) */
    for (i = nS; i > 0; i--) {
      computeGivens(Q_[nS + nS * nSmax], Q_[nS + (i - 1) * nSmax],
                    Q_[nS + nS * nSmax], Q_[nS + (i - 1) * nSmax], c, s);
      nu = s / (1.0 + c);
      for (j = 0; j < nS; j++)
        applyGivens(c, s, nu, Q_[j + nS * nSmax], Q_[j + (i - 1) * nSmax],
                    Q_[j + nS * nSmax], Q_[j + (i - 1) * nSmax]);

      /* Simultaneously transform R (coldim is already one less than Q) */
      for (j = i - 1; j < nS; j++)
        applyGivens(c, s, nu, R_[nS + j * nSmax], R_[(i - 1) + j * nSmax],
                    R_[nS + j * nSmax], R_[(i - 1) + j * nSmax]);
    }

    /* If we did an  odd permutation AND deleted a positive unity vector or
     * if we did an even permutation AND deleted a negative unity vector, then
     * det(Q)=-1
     * ->Change signs of first column of Q and first row of R s.t. we always
     * maintain det(Q)=1 */
...
returnValue SQProblemSchur::backsolveSchurQR(int_t dimS,
  ...
  if (INFO != 0) {
    ...
    if (INFO == ((long)0xDEADBEEF))
      MyPrintf("If SQProblemSchur is to be used, system LAPACK must be used "
               "instead of the qpOASES LAPACK replacement");
    return RET_QR_FACTORISATION_FAILED;
  }
...
returnValue SQProblemSchur::stepCalcRhs(
    int_t nFR, int_t nFX, int_t nAC, int_t* FR_idx, int_t* FX_idx,
    int_t* AC_idx, real_t& rhs_max, const real_t* const delta_g,
  ...
  if ((hessianType != HST_IDENTITY) && (hessianType != HST_ZERO)) {
    ...
    H->times(bounds.getFree(), bounds.getFixed(), 1, 1.0, delta_xFX, nFX, 1.0,
             tempA, nFR);
  }
  /* tempB becomes RHS for reduced augmented system, bA-A_CX*delta_xFR */
  A->times(constraints.getActive(), bounds.getFixed(), 1, -1.0, delta_xFX, nFX,
           1.0, tempB, nAC);

  /* If iterative refinement is requested, compute max-norm of RHS for
   * termination test. */
  rhs_max = 0.0;
...
returnValue SQProblemSchur::stepCalcReorder(
    int_t nFR, int_t nAC, int_t* FR_idx, int_t* AC_idx, int_t nFRStart,
    int_t nACStart, int_t* FR_idxStart, int_t* AC_idxStart, int_t* FR_iSort,
    int_t* FR_iSortStart, int_t* AC_iSort, int_t* AC_iSortStart, real_t* rhs) {
  int_t i, ii;
...
returnValue SQProblemSchur::stepCalcBacksolveSchur(int_t nFR, int_t nFX,
                                                   int_t nAC, int_t* FR_idx,
                                                   int_t* FX_idx, int_t* AC_idx,
                                                   int_t dim, real_t* rhs,
                                                   real_t* sol) {
  returnValue retval;
  int_t i, ii;
  ...
  for (ii = 0; ii < nS; ii++) {
    ...
    switch (schurUpdate[ii]) // TODO: All the loops below could be done faster
                             // by binary search or so
    {
    case SUT_VarFixed:
      q[ii] = 0.0;
      break;
  ...
  if (retval != SUCCESSFUL_RETURN) {
    ...
    return THROWERROR(
        RET_MATRIX_FACTORISATION_FAILED); // TODO: Different return code
  }

  /* Transfer extra compoments of the Schur complement solution to the correct
   * place. */
  for (ii = 0; ii < nS; ii++) {
    int_t idx = schurUpdateIndex[ii];
    switch (schurUpdate[ii]) // TODO: All the loops below could be done faster
...
returnValue SQProblemSchur::stepCalcReorder2(
    int_t nFR, int_t nAC, int_t* FR_idx, int_t* AC_idx, int_t nFRStart,
    int_t nACStart, int_t* FR_idxStart, int_t* AC_idxStart, int_t* FR_iSort,
    int_t* FR_iSortStart, int_t* AC_iSort, int_t* AC_iSortStart, real_t* sol,
    real_t* const delta_xFR, real_t* const delta_yAC) {
  int_t i, ii;
  i = 0;
  ii = 0;
  ...
  /* Transfer Schur complement solution for the active constraint multipliers to
   * the correct places */
  i = 0;
  ii = 0;
...
returnValue SQProblemSchur::stepCalcResid(
    int_t nFR, int_t nFX, int_t nAC, int_t* FR_idx, int_t* FX_idx,
    int_t* AC_idx, BooleanType Delta_bC_isZero, real_t* const delta_xFX,
    real_t* const delta_xFR, real_t* const delta_yAC,
    const real_t* const delta_g, const real_t* const delta_lbA,
    const real_t* const delta_ubA, real_t& rnrm) {
  int_t i, ii;
  ...
  default:
    H->times(bounds.getFree(), bounds.getFree(), 1, 1.0, delta_xFR, nFR, 1.0,
             tempA, nFR);
    H->times(bounds.getFree(), bounds.getFixed(), 1, 1.0, delta_xFX, nFX, 1.0,
             tempA, nFR);
    break;
  ...
  A->transTimes(constraints.getActive(), bounds.getFree(), 1, -1.0, delta_yAC,
                nAC, 1.0, tempA, nFR);
  rnrm = 0.0;
  for (i = 0; i < nFR; ++i)
  ...
  A->times(constraints.getActive(), bounds.getFree(), 1, -1.0, delta_xFR, nFR,
           1.0, tempB, nAC);
...
returnValue SQProblemSchur::stepCalcDeltayFx(
    int_t nFR, int_t nFX, int_t nAC, int_t* FX_idx, const real_t* const delta_g,
    real_t* const delta_xFX, real_t* const delta_xFR, real_t* const delta_yAC,
    real_t* const delta_yFX) {
  int_t i;
  for (i = 0; i < nFX; ++i)
  ...
  A->transTimes(constraints.getActive(), bounds.getFixed(), 1, -1.0, delta_yAC,
                nAC, 1.0, delta_yFX, nFX);
  ...
  if (hessianType == HST_ZERO) {
    ...
    H->times(bounds.getFixed(), bounds.getFree(), 1, 1.0, delta_xFR, nFR, 1.0,
             delta_yFX, nFX);
    H->times(bounds.getFixed(), bounds.getFixed(), 1, 1.0, delta_xFX, nFX, 1.0,
             delta_yFX, nFX);
  }
  return SUCCESSFUL_RETURN;
}

returnValue SQProblemSchur::determineStepDirection(
    const real_t* const delta_g, const real_t* const delta_lbA,
  ...
  returnValue retval = determineStepDirection2(
  ...
  if (retval == RET_QR_FACTORISATION_FAILED) {
    retval = resetSchurComplement(BT_FALSE);
    if (retval != SUCCESSFUL_RETURN) {
      MyPrintf("In SQProblem::determineStepDirection, resetSchurComplement "
               "returns %d\n",
               retval);
      return THROWERROR(retval);
    }
    retval = determineStepDirection2(
        delta_g, delta_lbA, delta_ubA, delta_lb, delta_ub, Delta_bC_isZero,
        Delta_bB_isZero, delta_xFX, delta_xFR, delta_yAC, delta_yFX);
  }
  return retval;
 ...
 */
returnValue SQProblemSchur::determineStepDirection2(
    const real_t* const delta_g, const real_t* const delta_lbA,
    const real_t* const delta_ubA, const real_t* const delta_lb,
    const real_t* const delta_ub, BooleanType Delta_bC_isZero,
    BooleanType Delta_bB_isZero, real_t* const delta_xFX,
  ...
  /* The linear system to be solved here is this:
     ...
     | A_CF  A_CX    0     0 | | -delta_yAC | = |  delta_bA  |  <-- active
     entries of delta_lbA and delta_ubA with corresponding sign \  0     I 0 0 /
     \ -delta_yFX /   \  delta_bX  /  <-- fixed entries of delta_lb and delta_ub
  ...
  */
  ...
  if (nFR + nAC > 0) {
    real_t rhs_max = 0.0;
    retval = stepCalcRhs(nFR, nFX, nAC, FR_idx, FX_idx, AC_idx, rhs_max,
    ...
    for (r = 0; r <= options.numRefinementSteps; ++r) {
      retval = stepCalcReorder(nFR, nAC, FR_idx, AC_idx, nFRStart, nACStart,
                               FR_idxStart, AC_idxStart, FR_iSort,
                               FR_iSortStart, AC_iSort, AC_iSortStart, rhs);
      if (retval != SUCCESSFUL_RETURN)
        return retval;
      ...
      if (nS > 0) {
        retval = stepCalcBacksolveSchur(nFR, nFX, nAC, FR_idx, FX_idx, AC_idx,
                                        dim, rhs, sol);
        if (retval != SUCCESSFUL_RETURN)
          return retval;
      }

      /* Transfer Schur complement solution for the free variables to the
       * correct places */
      retval =
          stepCalcReorder2(nFR, nAC, FR_idx, AC_idx, nFRStart, nACStart,
                           FR_idxStart, AC_idxStart, FR_iSort, FR_iSortStart,
                           AC_iSort, AC_iSortStart, sol, delta_xFR, delta_yAC);
      if (retval != SUCCESSFUL_RETURN)
        return retval;

      if (r < options.numRefinementSteps) // TODO: use "<" to avoid computation
                                          // in last round
      {
        real_t rnrm;
        retval = stepCalcResid(nFR, nFX, nAC, FR_idx, FX_idx, AC_idx,
                               Delta_bC_isZero, delta_xFX, delta_xFR, delta_yAC,
                               delta_g, delta_lbA, delta_ubA, rnrm);
        if (retval != SUCCESSFUL_RETURN)
          return retval;

        /* early termination of residual norm small enough */
        if (options.printLevel == PL_HIGH)
          MyPrintf("In iterative refinement (iter %d) rnrm = %e and "
                   "epsIterRef*rhs_max = %e.\n",
  ...
  if (nFX > 0) {
    retval = stepCalcDeltayFx(nFR, nFX, nAC, FX_idx, delta_g, delta_xFX,
...
returnValue
SQProblemSchur::resetSchurComplement(BooleanType allowInertiaCorrection) {
  int_t j;
  int_t nFR = getNFR();
  int_t nAC = getNAC();
  ...
  default:
    H->getSparseSubmatrix(bounds.getFree(), bounds.getFree(), 1, 1, numNonzeros,
                          0, 0, 0, BT_TRUE);
    break;
  }
  ...
  if (constraintProduct != 0) {
    MyPrintf("In SQProblemSchur::determineStepDirection, constraintProduct not "
             "yet implemented.\n");
    return THROWERROR(RET_NOT_YET_IMPLEMENTED);
  }
  A->getSparseSubmatrix(constraints.getActive(), bounds.getFree(), nFR + 1, 1,
                        numNonzerosA, 0, 0, 0, BT_FALSE);
  ...
  A->getSparseSubmatrix(constraints.getActive(), bounds.getFree(), nFR + 1, 1,
                        numNonzerosA, irn + numNonzeros, jcn + numNonzeros,
                        avals + numNonzeros, BT_FALSE);
  numNonzeros += numNonzerosA;

  // Call the linear solver
  sparseSolver->reset();
  returnValue retval =
      sparseSolver->setMatrixData(dim, numNonzeros, irn, jcn, avals);
  delete[] jcn;
  delete[] irn;
  delete[] avals;
  ...
  // If matrix is singular, add bounds/remove constraints according to zero
  // pivots
  ...
  if (retval == SUCCESSFUL_RETURN && allowInertiaCorrection) {
    ...
    if (neig > getNAC()) {
      if (options.printLevel == PL_HIGH)
        MyPrintf("WARNING: After new factorization, reduced Hessian has %i "
...
returnValue SQProblemSchur::computeMTimes(real_t alpha, const real_t* const x_,
                                          real_t beta, real_t* const y_) {
...
returnValue SQProblemSchur::computeMTransTimes(real_t alpha,
                                               const real_t* const x_,
                                               real_t beta, real_t* const y_) {
  if (isEqual(alpha, 1.0) == BT_FALSE ||
      (isZero(beta) == BT_FALSE && isEqual(beta, -1.0) == BT_FALSE))
    return THROWERROR(RET_NOT_YET_IMPLEMENTED);
...
returnValue SQProblemSchur::addToSchurComplement(
    int_t number, SchurUpdateType update, int_t numNonzerosM,
    const sparse_int_t* Mpos, const real_t* const Mvals, int_t numNonzerosN,
    const sparse_int_t* Npos, const real_t* const Nvals, real_t N_diag) {
  int_t i;
  ...
  if (retval != SUCCESSFUL_RETURN) {
    MyPrintf("sparseSolver->solve in SQProblemSchur::addToSchurComplement "
             "failed.\n");
    return THROWERROR(
        RET_MATRIX_FACTORISATION_FAILED); // TODO: Different return code
  }
  ...
  if (M_physicallength < M_jc[nS] + numNonzerosM) {
    ...
    int_t M_physicallength_new =
        getMax(2 * M_physicallength, M_physicallength + 2 * numNonzerosM);
...
returnValue SQProblemSchur::deleteFromSchurComplement(int_t idx,
                                                      BooleanType allowUndo) {
  if (options.printLevel == PL_HIGH)
    MyPrintf(
        "deleting entry %d with idx = %d and type %d from Schur complement.",
        idx, schurUpdateIndex[idx], schurUpdate[idx]);
  ...
  if (idx != nS - 1) {
    ...
    /* Store deleted row/column in the last row/column of S, can retrieve it
     * from there later */
    if (allowUndo == BT_TRUE) {
      for (int_t i = 0; i < nS; i++) {
    ...
    /* Store deleted column of M in the last column, can retrieve it from there
     * later */
    if (allowUndo == BT_TRUE) {
...
returnValue SQProblemSchur::undoDeleteFromSchurComplement(int_t idx) {
  if (options.printLevel == PL_HIGH)
    MyPrintf("undo deletion of entry %d with idx = %d and type %d from Schur "
             "complement. nS = %i\n",
...
returnValue SQProblemSchur::correctInertia() {
  ...
  /* if this method is triggered after flipping bounds, inertia is now probably
   * correct */
  if (neig == getNAC())
    return SUCCESSFUL_RETURN;
  ...
  while (neig > getNAC() && k < nFR) {
    ...
    /* If it's linearly independent, fix the next free variable at the nearest
     * bound */
    number = freeBoundIdx[k];
    if (addBound_checkLI(number) == RET_LINEARLY_INDEPENDENT) {
      /* This is just heuristics: we need the bound which gives correct
       * multiplier sign */
    ...
    /* Case 1: Schur complement has been reset, check inertia of new
     * factorization */
    if (nS == 0)
      neig = sparseSolver->getNegativeEigenvalues();
     ...
     * This method is called after a factorization reset or after ONE bound has
     * been added */
  ...
  if (neig > getNAC()) {
    if (options.printLevel == PL_HIGH)
      MyPrintf("Added %i bounds but KKT matrix still has %i negative "
...
returnValue SQProblemSchur::repairSingularWorkingSet() {
  ...
  for (k = defect - 1; k > -1; k--) {
    ...
    if (zeroPivots[k] < nFR) {
      ...
      if (options.printLevel == PL_HIGH)
        MyPrintf("WARNING: KKT matrix singular! Add bound %i before "
                 "refactorization.\n",
                 number);
      ...
      /* Here we do not need to update the Schur complement because KKT matrix
       * is factorized afterwards */
      if (bounds.moveFreeToFixed(number, B_status) != SUCCESSFUL_RETURN)
    ...
    /* Linearly dependent row in the Jacobian: remove a constraint */
      ...
      if (options.printLevel == PL_HIGH)
        MyPrintf("WARNING: KKT matrix singular! Removing constraint %i before "
                 "refactorization.\n",
                 number);
  ...
  if (options.printLevel == PL_HIGH)
    MyPrintf("WARNING: KKT matrix singular! Removed %i constraints and added "
             "%i bounds before refactorization.\n",
