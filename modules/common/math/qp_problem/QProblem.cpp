 ...
 *	developed online active set strategy for parametric quadratic
 *programming.
 */

#include "modules/common/math/qp_problem/QProblem.hpp"
#include "modules/common/math/qp_problem/LapackBlasReplacement.hpp"
 ...
 *	Q P r o b l e m
 */
QProblem::QProblem(int_t _nV, int_t _nC, HessianType _hessianType,
                   BooleanType allocDenseMats)
 ...
 *	i n i t
 */
returnValue QProblem::init(SymmetricMatrix* _H, const real_t* const _g,
                           Matrix* _A, const real_t* const _lb,
                           const real_t* const _ub, const real_t* const _lbA,
                           const real_t* const _ubA, int_t& nWSR,
                           real_t* const cputime, const real_t* const xOpt,
  ...
  if ((xOpt == 0) && (yOpt != 0) &&
      ((guessedBounds != 0) || (guessedConstraints != 0)))
    return THROWERROR(RET_INVALID_ARGUMENTS);
  ...
  return solveInitialQP(xOpt, yOpt, guessedBounds, guessedConstraints, _R, nWSR,
                        cputime);
}
 ...
 *	i n i t
 */
returnValue QProblem::init(const real_t* const _H, const real_t* const _g,
                           const real_t* const _A, const real_t* const _lb,
                           const real_t* const _ub, const real_t* const _lbA,
                           const real_t* const _ubA, int_t& nWSR,
                           real_t* const cputime, const real_t* const xOpt,
                           const real_t* const yOpt,
 ...
 *	i n i t
 */
returnValue QProblem::init(const char* const H_file, const char* const g_file,
                           const char* const A_file, const char* const lb_file,
                           const char* const ub_file,
                           const char* const lbA_file,
                           const char* const ubA_file, int_t& nWSR,
                           real_t* const cputime, const real_t* const xOpt,
                           const real_t* const yOpt,
                           const Bounds* const guessedBounds,
                           const Constraints* const guessedConstraints,
  ...
  if (setupQPdataFromFile(H_file, g_file, A_file, lb_file, ub_file, lbA_file,
                          ubA_file) != SUCCESSFUL_RETURN)
    return THROWERROR(RET_UNABLE_TO_READ_FILE);

  if (R_file == 0) {
    /* 3) Call to main initialisation routine. */
    return solveInitialQP(xOpt, yOpt, guessedBounds, guessedConstraints, 0,
                          nWSR, cputime);
  } else {
    ...
    return solveInitialQP(xOpt, yOpt, guessedBounds, guessedConstraints, R,
                          nWSR, cputime);
  }
}
 ...
 */
returnValue
QProblem::hotstart(const real_t* const g_new, const real_t* const lb_new,
                   const real_t* const ub_new, const real_t* const lbA_new,
                   const real_t* const ubA_new, int_t& nWSR,
                   real_t* const cputime, const Bounds* const guessedBounds,
                   const Constraints* const guessedConstraints) {
  int_t i, nActiveFar;
  int_t nV = getNV();
  int_t nC = getNC();
  ...
  /* Possibly update working sets according to guesses for working sets of
   * bounds and constraints. */
  if ((guessedBounds != 0) || (guessedConstraints != 0)) {
    if (cputime != 0)
      starttime = getCPUtime();

    const Bounds* actualGuessedBounds =
        (guessedBounds != 0) ? guessedBounds : &bounds;
    const Constraints* actualGuessedConstraints =
        (guessedConstraints != 0) ? guessedConstraints : &constraints;

    if (setupAuxiliaryQP(actualGuessedBounds, actualGuessedConstraints) !=
        SUCCESSFUL_RETURN)
  ...
  if (areBoundsConsistent(lb_new, ub_new, lbA_new, ubA_new) !=
      SUCCESSFUL_RETURN)
  ...
  if (options.enableFarBounds == BT_FALSE) {
    ...
    if (ub_new)
      for (i = 0; i < nV; i++)
        if ((ub_new[i] < INFTY) && (ub_new[i] > farbound))
          farbound = ub_new[i];
    if (lb_new)
      for (i = 0; i < nV; i++)
        if ((lb_new[i] > -INFTY) && (lb_new[i] < -farbound))
          farbound = -lb_new[i];
    if (ubA_new)
      for (i = 0; i < nC; i++)
        if ((ubA_new[i] < INFTY) && (ubA_new[i] > farbound))
          farbound = ubA_new[i];
    if (lbA_new)
      for (i = 0; i < nC; i++)
        if ((lbA_new[i] > -INFTY) && (lbA_new[i] < -farbound))
          farbound = -lbA_new[i];

    updateFarBounds(farbound, nV + nC, lb_new, lb_new_far, ub_new, ub_new_far,
    ...
    for (;;) {
      ...
      /* Automatically call standard solveQP if regularisation is not active. */
      returnvalue = solveRegularisedQP(
          g_new, lb_new_far, ub_new_far, lbA_new_far, ubA_new_far, nWSR,
          pcputime_rem, nWSR_performed, isFirstCall);
      ...
      if (infeasible == BT_TRUE) {
        ...
        updateFarBounds(farbound, nV + nC, lb_new, lb_new_far, ub_new,
                        ub_new_far, lbA_new, lbA_new_far, ubA_new, ubA_new_far);
      } else if (status == QPS_SOLVED) {
        real_t tol = farbound / options.growFarBounds * options.boundTolerance;

        for (i = 0; i < nV; ++i) {
          if (((lb_new == 0) || (lb_new_far[i] > lb_new[i])) &&
              (getAbs(lb_new_far[i] - x[i]) < tol))
            ++nActiveFar;
          if (((ub_new == 0) || (ub_new_far[i] < ub_new[i])) &&
              (getAbs(ub_new_far[i] - x[i]) < tol))
            ++nActiveFar;
        }
        for (i = 0; i < nC; ++i) {
          if (((lbA_new == 0) || (lbA_new_far[i] > lbA_new[i])) &&
              (getAbs(lbA_new_far[i] - Ax[i]) < tol))
            ++nActiveFar;
          if (((ubA_new == 0) || (ubA_new_far[i] < ubA_new[i])) &&
              (getAbs(ubA_new_far[i] - Ax[i]) < tol))
            ++nActiveFar;
        }
  ...
  return (returnvalue != SUCCESSFUL_RETURN) ? THROWERROR(returnvalue)
                                            : returnvalue;
}
 ...
 */
returnValue
QProblem::hotstart(const char* const g_file, const char* const lb_file,
                   const char* const ub_file, const char* const lbA_file,
                   const char* const ubA_file, int_t& nWSR,
                   real_t* const cputime, const Bounds* const guessedBounds,
                   const Constraints* const guessedConstraints) {
  int_t nV = getNV();
  ...
  returnvalue =
      loadQPvectorsFromFile(g_file, lb_file, ub_file, lbA_file, ubA_file, g_new,
                            lb_new, ub_new, lbA_new, ubA_new);
  if (returnvalue != SUCCESSFUL_RETURN) {
    if (ubA_file != 0)
      delete[] ubA_new;
    if (lbA_file != 0)
 ...
 */
returnValue QProblem::solveCurrentEQP(const int_t n_rhs, const real_t* g_in,
                                      const real_t* lb_in, const real_t* ub_in,
                                      const real_t* lbA_in,
                                      const real_t* ubA_in, real_t* x_out,
                                      real_t* y_out) {
  if ((x_out == 0) || (y_out == 0))
    return THROWERROR(RET_INVALID_ARGUMENTS);
  ...
  for (ii = 0; ii < n_rhs; ++ii) {
    returnvalue = determineStepDirection(g_in, lbA_in, ubA_in, lb_in, ub_in,
                                         BT_FALSE, BT_FALSE, delta_xFX,
                                         delta_xFR, delta_yAC, delta_yFX);
 ...
 */
returnValue
QProblem::setConstraintProduct(ConstraintProduct* const _constraintProduct) {
...
returnValue QProblem::printProperties() {
  ...
  case HST_POSDEF_NULLSPACE:
    myPrintf("Hessian matrix is positive definite on null space of active "
             "constraints.\n");
    break;
  ...
  case QPS_HOMOTOPYQPSOLVED:
    myPrintf("Status of QP object: an intermediate QP along the homotopy path "
             "was solved.\n");
    break;
  ...
  case PL_DEBUG_ITER:
    myPrintf("Print level of QP object is set to display a tabular output for "
             "debugging.\n");
    break;
  ...
  case PL_MEDIUM:
    myPrintf("Print level of QP object is medium, i.e. error and warnings are "
             "printed.\n");
    break;

  case PL_HIGH:
    myPrintf("Print level of QP object is high, i.e. all available output is "
             "printed.\n");
    break;
 ...
 */
returnValue
QProblem::solveInitialQP(const real_t* const xOpt, const real_t* const yOpt,
                         const Bounds* const guessedBounds,
                         const Constraints* const guessedConstraints,
                         const real_t* const _R, int_t& nWSR,
  ...
  if (obtainAuxiliaryWorkingSet(xOpt, yOpt, guessedBounds, guessedConstraints,
                                &auxiliaryBounds,
                                &auxiliaryConstraints) != SUCCESSFUL_RETURN)
    return THROWERROR(RET_INIT_FAILED);
  ...
  if (setupAuxiliaryWorkingSet(&auxiliaryBounds, &auxiliaryConstraints,
                               BT_TRUE) != SUCCESSFUL_RETURN)
    return THROWERROR(RET_INIT_FAILED);
  ...
  if (setupAuxiliaryQPbounds(&auxiliaryBounds, &auxiliaryConstraints,
                             BT_TRUE) != SUCCESSFUL_RETURN) {
    delete[] ubA_original;
    delete[] lbA_original;
    delete[] ub_original;
  ...
  returnValue returnvalue = hotstart(g_original, lb_original, ub_original,
                                     lbA_original, ubA_original, nWSR, cputime);
...
returnValue QProblem::solveQP(const real_t* const g_new,
                              ...
                              const real_t* const ubA_new, int_t& nWSR,
                              real_t* const cputime, int_t nWSRperformed,
  ...
  status = QPS_PERFORMINGHOMOTOPY; // AW TODO: Not sure if this is too early,
                                   // but otherwise removeBounds will complain
  ...
  // (ckirches) disabled this, as inactive but tight bounds may become inactive
  // equalities
  //            which would then never become active again!
  /*
          if ( setupSubjectToType( lb_new,ub_new,lbA_new,ubA_new ) !=
     SUCCESSFUL_RETURN ) return THROWERROR( RET_HOTSTART_FAILED );
  */
  ...
  for (iter = nWSRperformed; iter < nWSR; ++iter) {
    tabularOutput.idxAddB = tabularOutput.idxRemB = tabularOutput.idxAddC =
        tabularOutput.idxRemC = -1;
    tabularOutput.excAddB = tabularOutput.excRemB = tabularOutput.excAddC =
        tabularOutput.excRemC = 0;

    if (isCPUtimeLimitExceeded(cputime, starttime, iter - nWSRperformed) ==
        BT_TRUE) {
      /* If CPU time limit is exceeded, stop homotopy loop immediately!
       * Assign number of working set recalculations (runtime measurement is
       * stopped later). */
      nWSR = iter;
      break;
    }
    ...
    if (isFirstCall == BT_TRUE)
      ...
      snprintf(messageString, MAX_STRING_LENGTH, "%d* ...", (int)iter);
    getGlobalMessageHandler()->throwInfo(RET_ITERATION_STARTED, messageString,
                                         __FUNC__, __FILE__, __LINE__,
                                         VS_VISIBLE);
#endif

    /* 2) Determination of shift direction of the gradient and the
     * (constraints') bounds. */
    returnvalue = determineDataShift(
        g_new, lbA_new, ubA_new, lb_new, ub_new, delta_g, delta_lbA, delta_ubA,
        delta_lb, delta_ub, Delta_bC_isZero, Delta_bB_isZero);
    if (returnvalue != SUCCESSFUL_RETURN) {
      delete[] delta_yAC;
      delete[] delta_yFX;
      delete[] delta_xFX;
      ...
      /* Assign number of working set recalculations and stop runtime
       * measurement. */
      nWSR = iter;
      if (cputime != 0)
        *cputime = getCPUtime() - starttime;
    ...
    returnvalue = determineStepDirection(
        delta_g, delta_lbA, delta_ubA, delta_lb, delta_ub, Delta_bC_isZero,
        Delta_bB_isZero, delta_xFX, delta_xFR, delta_yAC, delta_yFX);
    if (returnvalue != SUCCESSFUL_RETURN) {
      delete[] delta_yAC;
      delete[] delta_yFX;
      delete[] delta_xFX;
     ...
     *    This step along the homotopy path is also taken (without changing
     * working set). */
    ...
    homotopyLength =
        getRelativeHomotopyLength(g_new, lb_new, ub_new, lbA_new, ubA_new);
    if (homotopyLength <= options.terminationTolerance) {
      status = QPS_SOLVED;

      THROWINFO(RET_OPTIMAL_SOLUTION_FOUND);

      if (printIteration(iter, BC_idx, BC_status, BC_isBound, homotopyLength,
                         isFirstCall) != SUCCESSFUL_RETURN)
        THROWERROR(
            RET_PRINT_ITERATION_FAILED); /* do not pass this as return value! */
    ...
    if (returnvalue != SUCCESSFUL_RETURN) {
      ...
      /* ...unboundedness... */
      if (unbounded ==
          BT_TRUE) /* not necessary since objective function convex! */
    ...
    if ((options.enableCholeskyRefactorisation > 0) &&
        ((iter % options.enableCholeskyRefactorisation) == 0)) {
    ...
    /* 8) Perform Ramping Strategy on zero homotopy step or drift correction (if
     * desired). */
    if (BC_status != ST_UNDEFINED) {
      if ((tau <= EPS) && (options.enableRamping == BT_TRUE))
        performRamping();
      else if ((options.enableDriftCorrection > 0) &&
               ((iter + 1) % options.enableDriftCorrection == 0))
        performDriftCorrection(); /* always returns SUCCESSFUL_RETURN */
    } else // AW: Added this.  Otherwise, I observed that the gradient might
           // become incorrect
    {
  ...
  if (options.printLevel == PL_HIGH) {
    ...
    return getGlobalMessageHandler()->throwWarning(
        RET_MAX_NWSR_REACHED, messageString, __FUNC__, __FILE__, __LINE__,
        VS_VISIBLE);
#else
 ...
 */
returnValue QProblem::solveRegularisedQP(
    const real_t* const g_new, const real_t* const lb_new,
    const real_t* const ub_new, const real_t* const lbA_new,
    const real_t* const ubA_new, int_t& nWSR, real_t* const cputime,
    int_t nWSRperformed, BooleanType isFirstCall) {
  int_t i, step;
  int_t nV = getNV();

  /* Perform normal QP solution if QP has not been regularised. */
  if (usingRegularisation() == BT_FALSE)
    return solveQP(g_new, lb_new, ub_new, lbA_new, ubA_new, nWSR, cputime,
                   nWSRperformed, isFirstCall);
  ...
  if (cputime == 0) {
    returnvalue = solveQP(g_new, lb_new, ub_new, lbA_new, ubA_new, nWSR, 0,
                          nWSRperformed, isFirstCall);
  } else {
    cputime_cur = *cputime;
    returnvalue = solveQP(g_new, lb_new, ub_new, lbA_new, ubA_new, nWSR,
                          &cputime_cur, nWSRperformed, isFirstCall);
  }
  nWSR_total = nWSR;
  ...
  for (step = 0; step < options.numRegularisationSteps; ++step) {
    ...
    if (cputime == 0) {
      returnvalue = solveQP(gMod, lb_new, ub_new, lbA_new, ubA_new, nWSR, 0,
                            nWSR_total, isFirstCall);
    } else {
      cputime_cur = *cputime - cputime_total;
      returnvalue = solveQP(gMod, lb_new, ub_new, lbA_new, ubA_new, nWSR,
                            &cputime_cur, nWSR_total, isFirstCall);
    }
...
returnValue QProblem::updateActivitiesForHotstart(const real_t* const lb_new,
  ...
  for (i = 0; i < nV; i++) {
    ...
    if (lb_new[i] > -INFTY && lb[i] <= -INFTY) {
      /* Now a lower bound has become finite.  To avoid numerical issues, adjust
       * lb */
...
returnValue QProblem::setupSubjectToType(const real_t* const lb_new,
  ...
  if ((lbA_new != 0) && (ubA_new != 0)) {
    for (i = 0; i < nC; ++i) {
      ...
      if ((lbA_new[i] < -INFTY + options.boundTolerance) &&
          (ubA_new[i] > INFTY - options.boundTolerance) &&
          (options.enableFarBounds == BT_FALSE)) {
        constraints.setType(i, ST_UNBOUNDED);
      } else {
        if (options.enableEqualities &&
            lbA[i] > ubA[i] - options.boundTolerance &&
...
returnValue QProblem::computeProjectedCholesky() {
  ...
  case HST_ZERO:
    if (usingRegularisation() == BT_TRUE) {
      ...
      /* Code should not get here, as  nZ == 0  always holds for an LP (without
       * regularisation)! */
      if (nZ > 0)
  ...
  if (info > 0) {
    if (R[0] < 0.0) {
      ...
      options.epsRegularisation =
          getMin(-R[0] + options.epsRegularisation,
                 getSqrt(getAbs(options.epsRegularisation)));
    }
...
returnValue QProblem::obtainAuxiliaryWorkingSet(
    ...
    const Bounds* const guessedBounds,
    const Constraints* const guessedConstraints, Bounds* auxiliaryBounds,
    Constraints* auxiliaryConstraints) const {
  int_t i = 0;
  int_t nV = getNV();
  int_t nC = getNC();

  /* 1) Ensure that desiredBounds is allocated (and different from
   * guessedBounds). */
  if ((auxiliaryBounds == 0) || (auxiliaryBounds == guessedBounds))
    return THROWERROR(RET_INVALID_ARGUMENTS);

  if ((auxiliaryConstraints == 0) ||
      (auxiliaryConstraints == guessedConstraints))
    return THROWERROR(RET_INVALID_ARGUMENTS);

  SubjectToStatus guessedStatus;

  /* 2) Setup working set of bounds for auxiliary initial QP. */
  if (QProblemB::obtainAuxiliaryWorkingSet(
          xOpt, yOpt, guessedBounds, auxiliaryBounds) != SUCCESSFUL_RETURN)
...
#endif
      {
        if (auxiliaryConstraints->setupConstraint(i, guessedStatus) !=
            SUCCESSFUL_RETURN)
          return THROWERROR(RET_OBTAINING_WORKINGSET_FAILED);
      }
    }
...
/* Moreover, add all equality constraints if specified. */
     ...
     * start with empty working set (or implicitly fixed bounds and equality
     * constraints only) for auxiliary QP. */
    if ((xOpt == 0) && (yOpt == 0)) {
      for (i = 0; i < nC; ++i) {
...
returnValue QProblem::setupAuxiliaryWorkingSet(
    ...
    const Constraints* const auxiliaryConstraints, BooleanType setupAfresh) {
  int_t i;
  int_t nV = getNV();
  int_t nC = getNC();
  ...
  for (i = 0; i < nC; i++)
    ...
    //            constraints that are ST_EQUALITies or may just have become
    //            equalities
  ...
  if (setupAfresh == BT_FALSE) {
    ...
    for (i = 0; i < nC; ++i) {
      if ((constraints.getStatus(i) == ST_LOWER) &&
        ...
        if (removeConstraint(i, updateCholesky, BT_FALSE,
                             options.enableNZCTests) != SUCCESSFUL_RETURN)
    ...
    for (i = 0; i < nV; ++i) {
      if ((bounds.getStatus(i) == ST_LOWER) &&
        ...
        if (removeBound(i, updateCholesky, BT_FALSE, options.enableNZCTests) !=
            SUCCESSFUL_RETURN)
          return THROWERROR(RET_SETUP_WORKINGSET_FAILED);
  ...
  for (i = 0; i < nV; ++i) {
    // if ( ( bounds.getType( i ) == ST_EQUALITY ) && ( ( bounds.getStatus( i )
    // == ST_INACTIVE ) && ( auxiliaryBounds->getStatus( i ) != ST_INACTIVE ) )
    // )

    // (ckirches) force equalities active

    if ((bounds.getType(i) == ST_EQUALITY) &&
        (bounds.getStatus(i) == ST_INACTIVE)) {
      // assert ( auxiliaryBounds->getStatus( i ) != ST_INACTIVE );
      /* No check for linear independence necessary. */
      if (addBound(i, ST_LOWER, updateCholesky) !=
          SUCCESSFUL_RETURN) // was auxiliaryBounds->getStatus( i )
        return THROWERROR(RET_SETUP_WORKINGSET_FAILED);
    }
  }
  ...
  for (i = 0; i < nC; ++i) {
    // if ( ( constraints.getType( i ) == ST_EQUALITY ) && ( (
    // constraints.getStatus( i ) == ST_INACTIVE ) && (
    // auxiliaryConstraints->getStatus( i ) != ST_INACTIVE ) ) )
    ...
    if ((constraints.getType(i) == ST_EQUALITY) &&
      ...
      if (addConstraint_checkLI(i) == RET_LINEARLY_INDEPENDENT) {
        if (addConstraint(i, ST_LOWER, updateCholesky) !=
            SUCCESSFUL_RETURN) // was auxiliaryConstraints->getStatus( i )
          return THROWERROR(RET_SETUP_WORKINGSET_FAILED);
      } else {
...
returnValue QProblem::setupAuxiliaryQPsolution(const real_t* const xOpt,
   ...
   *  old solution vector is kept if pointer to internal solution vector is
   * passed. */
  if (xOpt != 0) {
    if (xOpt != x)
      for (i = 0; i < nV; ++i)
 ...
 */
returnValue QProblem::areBoundsConsistent(const real_t* const lb_new,
                                          const real_t* const ub_new,
                                          const real_t* const lbA_new,
                                          const real_t* const ubA_new) const {
 ...
 */
returnValue
QProblem::setupAuxiliaryQPbounds(const Bounds* const auxiliaryBounds,
...
returnValue QProblem::addConstraint(int_t number, SubjectToStatus C_status,
  ...
  if (nZ > 0) {
    ...
    for (j = 0; j < nZ - 1; ++j) {
      ...
      for (i = 0; i < nFR; ++i) {
        ii = FR_idx[i];
        applyGivens(c, s, nu, QQ(ii, 1 + j), QQ(ii, j), QQ(ii, 1 + j),
                    QQ(ii, j));
      }

      if ((updateCholesky == BT_TRUE) && (hessianType != HST_ZERO) &&
...
returnValue QProblem::addConstraint_checkLI(int_t number) {
  ...
  if (options.enableFullLITests) {
    ...
    returnValue dsdreturnvalue =
        determineStepDirection(delta_g, nul, nul, nul, nul, BT_FALSE, BT_FALSE,
                               delta_xFX, delta_xFR, delta_yAC, delta_yFX);
...
returnValue QProblem::addConstraint_ensureLI(int_t number,
  ...
  A->transTimes(constraints.getActive(), bounds.getFixed(), 1, -1.0, xiC, nAC,
                1.0, xiB, nFX);
  ...
  performRatioTest(nAC, AC_idx, &constraints, num, xiC, options.epsNum,
                   options.epsDen, y_min, y_min_number);
  ...
  performRatioTest(nFX, FX_idx, &bounds, num, xiB, options.epsNum,
                   options.epsDen, y_min, y_min_number_bound);
  ...
  if (y_min_number >= 0) {
    ...
    if (y_min_isBound == BT_TRUE) {
#ifndef __SUPPRESSANYOUTPUT__
      snprintf(messageString, MAX_STRING_LENGTH, "bound no. %d.",
               (int)y_min_number);
      getGlobalMessageHandler()->throwInfo(RET_REMOVE_FROM_ACTIVESET,
                                           messageString, __FUNC__, __FILE__,
                                           __LINE__, VS_VISIBLE);
#endif

      if (removeBound(y_min_number, BT_TRUE, BT_FALSE, BT_FALSE) !=
          SUCCESSFUL_RETURN) {
...
#ifndef __SUPPRESSANYOUTPUT__
      snprintf(messageString, MAX_STRING_LENGTH, "constraint no. %d.",
               (int)y_min_number);
      getGlobalMessageHandler()->throwInfo(RET_REMOVE_FROM_ACTIVESET,
                                           messageString, __FUNC__, __FILE__,
                                           __LINE__, VS_VISIBLE);
      ...
      if (removeConstraint(y_min_number, BT_TRUE, BT_FALSE, BT_FALSE) !=
          SUCCESSFUL_RETURN) {
        returnvalue = RET_REMOVE_FROM_ACTIVESET_FAILED;
        goto farewell;
      }
...
farewell:
  ...
  getGlobalMessageHandler()->throwInfo(RET_LI_RESOLVED, 0, __FUNC__, __FILE__,
                                       __LINE__, VS_VISIBLE);
...
returnValue QProblem::addBound(int_t number, SubjectToStatus B_status,
                  ...
                  nothing to do */
  {
...

    {
      ...
      applyGivens(c, s, nu, TT(nAC - 1, tcol), tmp[nAC - 1], tmp[nAC - 1],
                  TT(nAC - 1, tcol));
    }
    ...
    for (j = nZ; j < nFR - 1; ++j) {
      ...
      for (i = (nFR - 2 - j); i < nAC; ++i)
        applyGivens(c, s, nu, TT(i, 1 + tcol - nZ + j), tmp[i], tmp[i],
                    TT(i, 1 + tcol - nZ + j));
    }
 ...
 */
returnValue QProblem::removeConstraint(int_t number, BooleanType updateCholesky,
  ...
  if (ensureNZC == BT_TRUE) {
    returnvalue =
        ensureNonzeroCurvature(BT_FALSE, number, exchangeHappened,
                               addBoundNotConstraint, addIdx, addStatus);
  ...
  if ((exchangeHappened == BT_FALSE) &&
      (options.enableFlippingBounds == BT_TRUE) && (allowFlipping == BT_TRUE))
  ...
  if (number_idx < nAC - 1) {
    ...
    for (j = (nAC - 2 - number_idx); j >= 0; --j) {
      computeGivens(TT(nAC - 2 - j, tcol + 1 + j), TT(nAC - 2 - j, tcol + j),
                    TT(nAC - 2 - j, tcol + 1 + j), TT(nAC - 2 - j, tcol + j), c,
                    s);
      nu = s / (1.0 + c);

      for (i = (nAC - j - 1); i < (nAC - 1); ++i)
        applyGivens(c, s, nu, TT(i, tcol + 1 + j), TT(i, tcol + j),
                    TT(i, tcol + 1 + j), TT(i, tcol + j));

      for (i = 0; i < nFR; ++i) {
        ii = FR_idx[i];
        applyGivens(c, s, nu, QQ(ii, nZ + 1 + j), QQ(ii, nZ + j),
                    QQ(ii, nZ + 1 + j), QQ(ii, nZ + j));
      }
    }
  } else {
    /* gimmick: write zeros into the last row of T */
  ...
  if ((updateCholesky == BT_TRUE) && (hessianType != HST_ZERO) &&
    ...
    if ((options.enableFlippingBounds == BT_TRUE) &&
        (allowFlipping == BT_TRUE) && (exchangeHappened == BT_FALSE)) {
 ...
 */
returnValue QProblem::removeBound(int_t number, BooleanType updateCholesky,
                                  BooleanType allowFlipping,
                                  BooleanType ensureNZC) {
  int_t i, j, ii, jj;
  returnValue returnvalue = SUCCESSFUL_RETURN;
  ...
  if (ensureNZC == BT_TRUE) {
    returnvalue =
        ensureNonzeroCurvature(BT_TRUE, number, exchangeHappened,
                               addBoundNotConstraint, addIdx, addStatus);
  ...
  if (nAC > 0) {
    ...
    for (j = (nAC - 1); j >= 0; --j) {
      computeGivens(tmp[nAC - 1 - j], TT(nAC - 1 - j, tcol + j),
  ...
  if ((updateCholesky == BT_TRUE) && (hessianType != HST_ZERO) &&
    ...
    if (nFR > 0) {
      ...
      H->times(bounds.getFree(), bounds.getFree(), 1, 1.0, z, nFR + 1, 0.0, Hz,
               nFR + 1);
...
returnValue QProblem::performPlainRatioTest(int_t nIdx,
                                            ...
                                            real_t epsNum, real_t epsDen,
                                            real_t& t, int_t& BC_idx) const {
  int_t i;
  for (i = 0; i < nIdx; i++)
...
returnValue
QProblem::ensureNonzeroCurvature(BooleanType removeBoundNotConstraint,
                                 int_t remIdx, BooleanType& exchangeHappened,
                                 BooleanType& addBoundNotConstraint,
                                 int_t& addIdx, SubjectToStatus& addStatus) {
  int_t i, ii;
  int_t addLBndIdx = -1, addLCnstrIdx = -1, addUBndIdx = -1,
        addUCnstrIdx = -1; /* exchange indices */
  int_t *FX_idx, *AC_idx, *IAC_idx;
  returnValue returnvalue = SUCCESSFUL_RETURN;
  ...
  if (removeBoundNotConstraint) {
    ...
    returnvalue =
        determineStepDirection(nul, nul, nul, ek, ek, BT_FALSE, BT_FALSE,
                               delta_xFX, delta_xFR, delta_yAC, delta_yFX);
    delete[] ek;
    delete[] nul;
  } else {
    ...
    returnvalue =
        determineStepDirection(nul, ek, ek, nul, nul, BT_FALSE, BT_TRUE,
                               delta_xFX, delta_xFR, delta_yAC, delta_yFX);
    delete[] ek;
    delete[] nul;
  }
  ...
  if (normXi < options.epsNZCTests * normS) {
    ...
    /* performRatioTest( nFR,FR_idx,&bounds, x_W,delta_xFR,
     * options.epsNum,options.epsDen, sigmaUBnd,addUBndIdx ); */
    sigmaUBnd = options.maxPrimalJump;
    addUBndIdx = -1;
    performPlainRatioTest(nFR, FR_idx, x_W, delta_xFR, options.epsNum,
                          options.epsDen, sigmaUBnd, addUBndIdx);
    if (removeBoundNotConstraint == BT_TRUE &&
        bounds.getStatus(remIdx) == ST_LOWER) {
      /* also consider bound which is to be removed */
      real_t one = 1.0;
      x_W[0] = ub[remIdx] - x[remIdx];
      performPlainRatioTest(1, &remIdx, x_W, &one, options.epsNum,
                            options.epsDen, sigmaUBnd, addUBndIdx);
    }
    ...
    /* performRatioTest( nFR,FR_idx,&bounds, x_W,delta_xFR,
     * options.epsNum,options.epsDen, sigmaLBnd,addLBndIdx ); */
    sigmaLBnd = options.maxPrimalJump;
    addLBndIdx = -1;
    performPlainRatioTest(nFR, FR_idx, x_W, delta_xFR, options.epsNum,
                          options.epsDen, sigmaLBnd, addLBndIdx);
    if (removeBoundNotConstraint == BT_TRUE &&
        bounds.getStatus(remIdx) == ST_UPPER) {
      /* also consider bound which is to be removed */
      real_t one = 1.0;
    ...
    A->times(constraints.getInactive(), bounds.getFixed(), 1, 1.0, delta_xFX,
             nFX, 0.0, As, nIAC);
    A->times(constraints.getInactive(), bounds.getFree(), 1, 1.0, delta_xFR,
             nFR, 1.0, As, nIAC);
    ...
    /* performRatioTest( nIAC,IAC_idx,&constraints, Ax_W,As,
     * options.epsNum,options.epsDen, sigmaUCnstr,addUCnstrIdx ); */
    sigmaUCnstr = options.maxPrimalJump;
    addUCnstrIdx = -1;
    performPlainRatioTest(nIAC, IAC_idx, Ax_W, As, options.epsNum,
                          options.epsDen, sigmaUCnstr, addUCnstrIdx);
    if (removeBoundNotConstraint == BT_FALSE &&
        constraints.getStatus(remIdx) == ST_LOWER) {
      /* also consider constraint which is to be removed */
      real_t one = 1.0;
      performPlainRatioTest(1, &remIdx, &Ax_u[remIdx], &one, options.epsNum,
                            options.epsDen, sigmaUCnstr, addUCnstrIdx);
    }
    ...
    for (i = 0; i < nIAC; i++)
      As[i] = -As[i];
    /* performRatioTest( nIAC,IAC_idx,&constraints, Ax_W,As,
     * options.epsNum,options.epsDen, sigmaLCnstr,addLCnstrIdx ); */
    sigmaLCnstr = options.maxPrimalJump;
    addLCnstrIdx = -1;
    performPlainRatioTest(nIAC, IAC_idx, Ax_W, As, options.epsNum,
                          options.epsDen, sigmaLCnstr, addLCnstrIdx);
    if (removeBoundNotConstraint == BT_FALSE &&
        constraints.getStatus(remIdx) == ST_UPPER) {
      /* also consider constraint which is to be removed */
      real_t one = 1.0;
      performPlainRatioTest(1, &remIdx, &Ax_l[remIdx], &one, options.epsNum,
                            options.epsDen, sigmaLCnstr, addLCnstrIdx);
    }

    /* perform primal jump */
    sigma = options.maxPrimalJump;
    if (sigmaUCnstr < sigma) {
      sigma = sigmaUCnstr;
      addStatus = ST_UPPER;
      addBoundNotConstraint = BT_FALSE;
      addIdx = addUCnstrIdx;
    }
    if (sigmaLCnstr < sigma) {
      sigma = sigmaLCnstr;
      addStatus = ST_LOWER;
      addBoundNotConstraint = BT_FALSE;
      addIdx = addLCnstrIdx;
 ...
 *	b a c k s o l v e T
 */
returnValue QProblem::backsolveT(const real_t* const b, BooleanType transposed,
                                 real_t* const a) const {
  int_t i, j;
  int_t nT = getNAC();
  int_t tcol = sizeT - nT;
 ...
 */
returnValue QProblem::determineDataShift(
    const real_t* const g_new, const real_t* const lbA_new,
    const real_t* const ubA_new, const real_t* const lb_new,
    const real_t* const ub_new, real_t* const delta_g, real_t* const delta_lbA,
    real_t* const delta_ubA, real_t* const delta_lb, real_t* const delta_ub,
  ...
  QProblemB::determineDataShift(g_new, lb_new, ub_new, delta_g, delta_lb,
                                delta_ub, Delta_bB_isZero);

  /* II) DETERMINE DATA SHIFT FOR CONSTRAINTS */
  /* 1) Calculate shift directions. */
  for (i = 0; i < nC; ++i) {
    /* if lower constraints' bounds are to be disabled or do not exist, shift
     * them to -infinity */
    if (lbA_new != 0)
  ...
  for (i = 0; i < nC; ++i) {
    /* if upper constraints' bounds are to be disabled or do not exist, shift
     * them to infinity */
    if (ubA_new != 0)
 ...
 */
returnValue QProblem::determineStepDirection(
    const real_t* const delta_g, const real_t* const delta_lbA,
    const real_t* const delta_ubA, const real_t* const delta_lb,
    const real_t* const delta_ub, BooleanType Delta_bC_isZero,
    BooleanType Delta_bB_isZero, real_t* const delta_xFX,
    real_t* const delta_xFR, real_t* const delta_yAC, real_t* const delta_yFX) {
  int_t i, j, ii, jj, r;
  int_t nV = getNV();
  int_t nFR = getNFR();
  int_t nFX = getNFX();
  ...
  for (r = 0; r <= options.numRefinementSteps; ++r) {
      ...
      if (nAC > 0) {
        if ((Delta_bC_isZero == BT_TRUE) && (Delta_bB_isZero == BT_TRUE)) {
          ...
          if ((Delta_bB_isZero == BT_FALSE) && (r == 0))
            A->times(constraints.getActive(), bounds.getFixed(), 1, -1.0,
                     delta_xFX, nFX, 1.0, tempB, nAC);
      ...
      if ((hessianType == HST_ZERO) || (hessianType == HST_IDENTITY)) {
        ...
        if (hessianType == HST_ZERO) {
          if (usingRegularisation() == BT_TRUE) {
            ...
            /* When solving LPs without regularisation, iterates must always be
             * at a vertex. */
            if (nZ > 0)
              return THROWERROR(RET_UNKNOWN_BUG);
          }
        ...
        if ((Delta_bB_isZero == BT_FALSE) && (r == 0))
          H->times(bounds.getFree(), bounds.getFixed(), 1, 1.0, delta_xFX, nFX,
                   1.0, tempA, nFR);

        /* compute HFR*delta_xFRy */
        if ((nAC > 0) &&
            ((Delta_bC_isZero == BT_FALSE) || (Delta_bB_isZero == BT_FALSE)))
          H->times(bounds.getFree(), bounds.getFree(), 1, 1.0, delta_xFR_TMP,
                   nFR, 1.0, tempA, nFR);

        /* compute ZFR_delta_xFRz = (Z'*HFR*Z) \ Z * (HFR*delta_xFR +
         * HMX*delta_xFX + delta_gFR) */
        if (nZ > 0) {
          for (j = 0; j < nFR; ++j) {
            jj = FR_idx[j];
            for (i = 0; i < nZ; ++i)
    ...
    if (nAC > 0) /* => ( nFR = nZ + nAC > 0 ) */
    {
      if ((hessianType == HST_ZERO) || (hessianType == HST_IDENTITY)) {
        ...
        if (nZ > 0)
          H->times(bounds.getFree(), bounds.getFree(), 1, 1.0, ZFR_delta_xFRz,
                   nFR, 1.0, tempA, nFR);
    ...
    if (options.numRefinementSteps > 0) {
      ...
      default:
        H->times(bounds.getFree(), bounds.getFree(), 1, 1.0, delta_xFR, nFR,
                 1.0, tempA, nFR);
        H->times(bounds.getFree(), bounds.getFixed(), 1, 1.0, delta_xFX, nFX,
                 1.0, tempA, nFR);
        break;
      ...
      A->transTimes(constraints.getActive(), bounds.getFree(), 1, -1.0,
                    delta_yAC, nAC, 1.0, tempA, nFR);
      real_t rnrm = 0.0;
      for (i = 0; i < nFR; ++i)
      ...
      A->times(constraints.getActive(), bounds.getFree(), 1, -1.0, delta_xFR,
               nFR, 1.0, tempB, nAC);
      A->times(constraints.getActive(), bounds.getFixed(), 1, -1.0, delta_xFX,
               nFX, 1.0, tempB, nAC);
      for (i = 0; i < nAC; ++i)
  ...
  if (nFX > 0) {
    ...
    A->transTimes(constraints.getActive(), bounds.getFixed(), 1, -1.0,
                  delta_yAC, nAC, 1.0, delta_yFX, nFX);
    ...
    default:
      H->times(bounds.getFixed(), bounds.getFree(), 1, 1.0, delta_xFR, nFR, 1.0,
               delta_yFX, nFX);
      H->times(bounds.getFixed(), bounds.getFixed(), 1, 1.0, delta_xFX, nFX,
               1.0, delta_yFX, nFX);
    }
  }
...
returnValue QProblem::performStep(
  ...
  performRatioTest(nAC, AC_idx, &constraints, num, den, options.epsNum,
                   options.epsDen, tau, BC_idx_tmp);
  ...
  performRatioTest(nFX, FX_idx, &bounds, num, den, options.epsNum,
                   options.epsDen, tau, BC_idx_tmp);
  ...
  if (constraintProduct == 0) {
    A->times(constraints.getInactive(), 0, 1, 1.0, delta_x, nV, 0.0, delta_Ax,
             nC, BT_FALSE);
  } else {
    for (i = 0; i < nIAC; ++i) {
      ii = IAC_idx[i];
  ...
  if (constraints.hasNoLower() == BT_FALSE) {
    ...
    performRatioTest(nIAC, IAC_idx, &constraints, num, den, options.epsNum,
                     options.epsDen, tau, BC_idx_tmp);
  ...
  if (bounds.hasNoLower() == BT_FALSE) {
    ...
    performRatioTest(nFR, FR_idx, &bounds, num, den, options.epsNum,
                     options.epsDen, tau, BC_idx_tmp);
  ...
  if (BC_status == ST_UNDEFINED)
    ...
    snprintf(messageString, MAX_STRING_LENGTH,
             "Stepsize is %.15e! (idx = %d, isBound = %d, status = %d)", tau,
             (int)BC_idx, (int)BC_isBound, (int)BC_status);

  getGlobalMessageHandler()->throwInfo(RET_STEPSIZE_NONPOSITIVE, messageString,
                                       __FUNC__, __FILE__, __LINE__,
                                       VS_VISIBLE);
#endif
...
/* print a stepsize warning if stepsize is zero */
    ...
    getGlobalMessageHandler()->throwWarning(
        RET_STEPSIZE, messageString, __FUNC__, __FILE__, __LINE__, VS_VISIBLE);
#endif
  }
 ...
 */
returnValue QProblem::changeActiveSet(int_t BC_idx, SubjectToStatus BC_status,
                                      BooleanType BC_isBound) {
  int_t nV = getNV();
  ...
  case ST_INACTIVE:
    if (BC_isBound == BT_TRUE) {
      ...
      if (removeBound(BC_idx, BT_TRUE, BT_TRUE, options.enableNZCTests) !=
          SUCCESSFUL_RETURN)
      ...
      if (removeConstraint(BC_idx, BT_TRUE, BT_TRUE, options.enableNZCTests) !=
          SUCCESSFUL_RETURN)
        return THROWERROR(RET_REMOVE_FROM_ACTIVESET_FAILED);
  ...
  default:
    ...
    if (BC_isBound == BT_TRUE) {
      ...
      if (BC_status == ST_LOWER)
        snprintf(messageString, MAX_STRING_LENGTH, "lower bound no. %d.",
                 (int)BC_idx);
      else
        snprintf(messageString, MAX_STRING_LENGTH, "upper bound no. %d.",
                 (int)BC_idx);
      getGlobalMessageHandler()->throwInfo(RET_ADD_TO_ACTIVESET, messageString,
                                           __FUNC__, __FILE__, __LINE__,
                                           VS_VISIBLE);
#endif
      ...
      if (BC_status == ST_LOWER)
        snprintf(messageString, MAX_STRING_LENGTH,
                 "lower constraint's bound no. %d.", (int)BC_idx);
      else
        snprintf(messageString, MAX_STRING_LENGTH,
                 "upper constraint's bound no. %d.", (int)BC_idx);
      getGlobalMessageHandler()->throwInfo(RET_ADD_TO_ACTIVESET, messageString,
                                           __FUNC__, __FILE__, __LINE__,
                                           VS_VISIBLE);
#endif
...
returnValue QProblem::performRamping() {
  ...
  for (i = 0; i < nV; i++) {
    ...
    case ST_EQUALITY:
      lb[i] = x[i];
      ub[i] = x[i]; /* reestablish exact feasibility */
      continue;

    case ST_BOUNDED:
      tP = static_cast<real_t>((i + rampOffset) % nRamp) /
           static_cast<real_t>(nRamp - 1);
      rampValP = (1.0 - tP) * ramp0 + tP * ramp1;
      tD = static_cast<real_t>((nV + nC + nC + i + rampOffset) % nRamp) /
           static_cast<real_t>(nRamp - 1);
      rampValD = (1.0 - tD) * ramp0 + tD * ramp1;
      bstat = bounds.getStatus(i);
      if (bstat != ST_LOWER) {
        sca = getMax(getAbs(x[i]), 1.0);
        lb[i] = x[i] - sca * rampValP;
      }
      if (bstat != ST_UPPER) {
        sca = getMax(getAbs(x[i]), 1.0);
        ub[i] = x[i] + sca * rampValP;
      }
      if (bstat == ST_LOWER) {
        lb[i] = x[i];
        y[i] = +rampValD;
      ...
      if (bstat == ST_INACTIVE)
        y[i] = 0.0; /* reestablish exact complementarity */
      break;
  ...
  for (i = 0; i < nC; i++) {
    ...
    case ST_EQUALITY:
      lbA[i] = Ax[i];
      ubA[i] = Ax[i]; /* reestablish exact feasibility */
      continue;

    case ST_BOUNDED:
      tP = static_cast<real_t>((nV + i + rampOffset) % nRamp) /
           static_cast<real_t>(nRamp - 1);
      rampValP = (1.0 - tP) * ramp0 + tP * ramp1;
      tD = static_cast<real_t>((nV + nC + i + rampOffset) % nRamp) /
           static_cast<real_t>(nRamp - 1);
      rampValD = (1.0 - tD) * ramp0 + tD * ramp1;
      cstat = constraints.getStatus(i);
      if (cstat != ST_LOWER) {
        sca = getMax(getAbs(Ax[i]), 1.0);
        lbA[i] = Ax[i] - sca * rampValP;
      }
      if (cstat != ST_UPPER) {
        sca = getMax(getAbs(Ax[i]), 1.0);
        ubA[i] = Ax[i] + sca * rampValP;
      }
      if (cstat == ST_LOWER) {
        lbA[i] = Ax[i];
        y[nV + i] = +rampValD;
      ...
      if (cstat == ST_INACTIVE)
        y[nV + i] = 0.0; /* reestablish exact complementarity */
 ...
 */
returnValue QProblem::updateFarBounds(
    real_t curFarBound, int_t nRamp, const real_t* const lb_new,
    real_t* const lb_new_far, const real_t* const ub_new,
    real_t* const ub_new_far, const real_t* const lbA_new,
    real_t* const lbA_new_far, const real_t* const ubA_new,
    real_t* const ubA_new_far) const {
  int_t i;
  real_t rampVal, t;
  int_t nV = getNV();
  int_t nC = getNC();

  returnValue returnvalue = QProblemB::updateFarBounds(
  ...
  if (options.enableRamping == BT_TRUE) {
    for (i = 0; i < nC; ++i) {
      t = static_cast<real_t>((nV + i + rampOffset) % nRamp) /
          static_cast<real_t>(nRamp - 1);
 ...
 */
returnValue
QProblem::setupAuxiliaryQP(const Bounds* const guessedBounds,
                           const Constraints* const guessedConstraints) {
  int_t i, j;
  int_t nV = getNV();
  int_t nC = getNC();
  ...
  if (shallRefactorise(guessedBounds, guessedConstraints) == BT_TRUE) {
    ...
    if (setupAuxiliaryWorkingSet(guessedBounds, guessedConstraints, BT_TRUE) !=
        SUCCESSFUL_RETURN)
      THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);

    /* 4) Computes Cholesky decomposition of projected Hessian
     *    This now handles all special cases (no active bounds/constraints, no
     * nullspace) */
    if (computeProjectedCholesky() != SUCCESSFUL_RETURN)
      return THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
  } else {
    /* ... WITHOUT REFACTORISATION: */
    if (setupAuxiliaryWorkingSet(guessedBounds, guessedConstraints, BT_FALSE) !=
        SUCCESSFUL_RETURN)
      THROWERROR(RET_SETUP_AUXILIARYQP_FAILED);
  }

  /* II) SETUP AUXILIARY QP DATA: */
  /* 1) Ensure that dual variable is zero for free bounds and inactive
   * constraints. */
  for (i = 0; i < nV; ++i)
    if (bounds.getStatus(i) == ST_INACTIVE)
      y[i] = 0.0;
...
BooleanType
QProblem::shallRefactorise(const Bounds* const guessedBounds,
                           const Constraints* const guessedConstraints) const {
  int_t i;
  int_t nV = getNV();
  int_t nC = getNC();
  ...
  if (2 * (differenceNumberBounds + differenceNumberConstraints) >
      guessedConstraints->getNAC() + guessedBounds->getNFX())
    return BT_TRUE;
  else
    return BT_FALSE;
}
 ...
 */
returnValue QProblem::setupQPdata(SymmetricMatrix* _H, const real_t* const _g,
                                  Matrix* _A, const real_t* const _lb,
 ...
 */
returnValue
QProblem::setupQPdata(const real_t* const _H, const real_t* const _g,
                      const real_t* const _A, const real_t* const _lb,
                      const real_t* const _ub, const real_t* const _lbA,
                      const real_t* const _ubA) {
  int_t nC = getNC();
...
returnValue QProblem::setupQPdataFromFile(const char* const H_file,
  ...
  /* 1) Load Hessian matrix as well as lower and upper bounds vectors from
   * files. */
  returnvalue =
      QProblemB::setupQPdataFromFile(H_file, g_file, lb_file, ub_file);
  if (returnvalue != SUCCESSFUL_RETURN)
    return THROWERROR(returnvalue);
 ...
 */
returnValue QProblem::loadQPvectorsFromFile(
    const char* const g_file, const char* const lb_file,
    const char* const ub_file, const char* const lbA_file,
    const char* const ubA_file, real_t* const g_new, real_t* const lb_new,
    real_t* const ub_new, real_t* const lbA_new, real_t* const ubA_new) const {
  int_t nC = getNC();

  returnValue returnvalue;

  /* 1) Load gradient vector as well as lower and upper bounds vectors from
   * files. */
  returnvalue = QProblemB::loadQPvectorsFromFile(g_file, lb_file, ub_file,
                                                 g_new, lb_new, ub_new);
  if (returnvalue != SUCCESSFUL_RETURN)
    return THROWERROR(returnvalue);
 ...
 */
returnValue QProblem::printIteration(int_t iter, int_t BC_idx,
                                     SubjectToStatus BC_status,
                                     BooleanType BC_isBound,
                                     real_t homotopyLength,
                                     BooleanType isFirstCall) {
  ...
  case PL_DEBUG_ITER:
    ...
    /* complementarity */
    for (i = 0; i < nV; i++)
      if (y[i] > +EPS && getAbs((lb[i] - x[i]) * y[i]) > bcmpl)
        bcmpl = getAbs((lb[i] - x[i]) * y[i]);
    for (i = 0; i < nV; i++)
      if (y[i] < -EPS && getAbs((ub[i] - x[i]) * y[i]) > bcmpl)
        bcmpl = getAbs((ub[i] - x[i]) * y[i]);
    for (i = 0; i < nC; i++)
      if (y[nV + i] > +EPS && getAbs((lbA[i] - AX[i]) * y[nV + i]) > ccmpl)
        ccmpl = getAbs((lbA[i] - AX[i]) * y[nV + i]);
    for (i = 0; i < nC; i++)
      if (y[nV + i] < -EPS && getAbs((ubA[i] - AX[i]) * y[nV + i]) > ccmpl)
        ccmpl = getAbs((ubA[i] - AX[i]) * y[nV + i]);
    ...
    if (tabularOutput.idxAddB >= 0) {
      snprintf(myPrintfString, MAX_STRING_LENGTH, "%4d ",
  ...
  case PL_TABULAR:
    ...
    if (tabularOutput.idxAddB >= 0) {
      snprintf(myPrintfString, MAX_STRING_LENGTH, "%5d%c ",
               (int)(tabularOutput.idxAddB), excStr[tabularOutput.excAddB]);
      myPrintf(myPrintfString);
    } else {
      myPrintf("       ");
    }
    ...
    if (tabularOutput.idxRemB >= 0) {
      snprintf(myPrintfString, MAX_STRING_LENGTH, "%5d%c ",
               (int)(tabularOutput.idxRemB), excStr[tabularOutput.excRemB]);
      myPrintf(myPrintfString);
    } else {
      myPrintf("       ");
    }
    ...
    if (tabularOutput.idxAddC >= 0) {
      snprintf(myPrintfString, MAX_STRING_LENGTH, "%5d%c ",
               (int)(tabularOutput.idxAddC), excStr[tabularOutput.excAddC]);
      myPrintf(myPrintfString);
    } else {
      myPrintf("       ");
    }
    ...
    if (tabularOutput.idxRemC >= 0) {
      snprintf(myPrintfString, MAX_STRING_LENGTH, "%5d%c ",
               (int)(tabularOutput.idxRemC), excStr[tabularOutput.excRemC]);
      myPrintf(myPrintfString);
    } else {
      myPrintf("       ");
    }
  ...
  case PL_MEDIUM:
    ...
    if (BC_status == ST_UNDEFINED) {
      ...
      int ret20 = snprintf(
          myPrintfString, MAX_STRING_LENGTH,
...
returnValue QProblem::dropInfeasibles(int_t BC_number,
                                      SubjectToStatus BC_status,
                                      BooleanType BC_isBound, real_t* xiB,
                                      real_t* xiC) {
  int_t i;

  int_t nAC = getNAC();
  int_t nFX = getNFX();
  int_t blockingPriority =
      (BC_isBound) ? options.dropBoundPriority : options.dropIneqConPriority;
  int_t y_min_number = -1;
  ...
  if (y_min_number >= 0) {
    ...
    if (y_min_isBound) {
      ...
      bounds.setStatus(y_min_number, (status_ == ST_LOWER)
                                         ? ST_INFEASIBLE_LOWER
                                         : ST_INFEASIBLE_UPPER);
      // TODO: fix duals y[]
      ...
      constraints.setStatus(y_min_number, (status_ == ST_LOWER)
                                              ? ST_INFEASIBLE_LOWER
                                              : ST_INFEASIBLE_UPPER);
      // TODO: fix duals y[]
    ...
    // ... now return, add the blocking constraint, and continue solving QP with
    // dropped bound/constraint
    return SUCCESSFUL_RETURN;
    ...
    if (BC_isBound)
      bounds.setStatus(BC_number, (BC_status == ST_LOWER)
                                      ? ST_INFEASIBLE_LOWER
                                      : ST_INFEASIBLE_UPPER);
    else
      constraints.setStatus(BC_number, (BC_status == ST_LOWER)
                                           ? ST_INFEASIBLE_LOWER
                                           : ST_INFEASIBLE_UPPER);
