...
returnValue QProblemB::init(const char* const H_file, const char* const g_file,
                            ...
                            const char* const ub_file, int_t& nWSR,
  ...
  if (setupQPdataFromFile(H_file, g_file, lb_file, ub_file) !=
      SUCCESSFUL_RETURN)
...
returnValue QProblemB::hotstart(const real_t* const g_new,
                                ...
                                const real_t* const ub_new, int_t& nWSR,
  ...
  if (options.enableFarBounds == BT_FALSE) {
    ...
    updateFarBounds(farbound, nV, lb_new, lb_new_far, ub_new, ub_new_far);
    ...
    for (;;) {
      ...
      /* Automatically call standard solveQP if regularisation is not active. */
      returnvalue =
          solveRegularisedQP(g_new, lb_new_far, ub_new_far, nWSR,
                             &cputime_remaining, nWSR_performed, isFirstCall);
...
returnValue QProblemB::hotstart(const char* const g_file,
  ...
  returnvalue =
      loadQPvectorsFromFile(g_file, lb_file, ub_file, g_new, lb_new, ub_new);
  if (returnvalue != SUCCESSFUL_RETURN) {
    if (ub_file != 0)
      delete[] ub_new;
    if (lb_file != 0)
  ...
  returnvalue = hotstart(g_new, lb_new, ub_new, nWSR, cputime, guessedBounds);
...
returnValue QProblemB::printOptions() const { return options.print(); }
...
returnValue QProblemB::determineHessianType() {
  ...
  /* 2) Otherwise it is diagonal and test for identity or zero matrix is
   * performed. */
...
returnValue QProblemB::setupSubjectToType(const real_t* const lb_new,
  ...
  if ((lb_new != 0) && (ub_new != 0)) {
    for (i = 0; i < nV; ++i) {
      if ((lb_new[i] < -INFTY + options.boundTolerance) &&
          (ub_new[i] > INFTY - options.boundTolerance) &&
...
returnValue QProblemB::obtainAuxiliaryWorkingSet(
    const real_t* const xOpt, const real_t* const yOpt,
    const Bounds* const guessedBounds, Bounds* auxiliaryBounds) const {
  int_t i = 0;
  int_t nV = getNV();
...
/* Only add all implictly fixed variables if specified. */
        ...
        default:
          if (auxiliaryBounds->setupBound(i, options.initialStatusBounds) !=
              SUCCESSFUL_RETURN)
            return THROWERROR(RET_OBTAINING_WORKINGSET_FAILED);
          break;
        }
 ...
 */
returnValue QProblemB::determineDataShift(
    const real_t* const g_new, const real_t* const lb_new,
    const real_t* const ub_new, real_t* const delta_g, real_t* const delta_lb,
    real_t* const delta_ub, BooleanType& Delta_bB_isZero) {
  int_t i, ii;
  int_t nV = getNV();
  int_t nFX = getNFX();
 ...
 */
returnValue QProblemB::areBoundsConsistent(const real_t* const lb_new,
                                           const real_t* const ub_new) const {
  if (lb_new && ub_new) {
 ...
 */
returnValue QProblemB::performRatioTest(int_t nIdx, const int_t* const idxList,
                                        const SubjectTo* const subjectTo,
                                        const real_t* const num,
                                        const real_t* const den, real_t epsNum,
                                        real_t epsDen, real_t& t,
                                        int_t& BC_idx) const {
  int_t i, ii;
  ...
  for (i = 0; i < nIdx; ++i) {
    ...
    if (subjectTo->getType(ii) != ST_EQUALITY) {
      if ((subjectTo->getStatus(ii) == ST_LOWER) ||
          (subjectTo->getStatus(ii) == ST_INACTIVE)) {
        if (isBlocking(num[i], den[i], epsNum, epsDen, t) == BT_TRUE) {
          t = num[i] / den[i];
          BC_idx = ii;
        }
      } else if (subjectTo->getStatus(ii) == ST_UPPER) {
        if (isBlocking(-num[i], -den[i], epsNum, epsDen, t) == BT_TRUE) {
          t = num[i] / den[i];
          BC_idx = ii;
        }
...
returnValue QProblemB::performRamping() {
  ...
  for (i = 0; i < nV; i++) {
    ...
    case ST_EQUALITY:
      lb[i] = x[i];
      ub[i] = x[i];
      continue; /* reestablish exact feasibility */
    case ST_UNBOUNDED:
      continue;
    case ST_DISABLED:
      continue;
    ...
    t = static_cast<real_t>((i + rampOffset) % nV) /
        static_cast<real_t>(nV - 1);
...
returnValue QProblemB::updateFarBounds(real_t curFarBound, int_t nRamp,
  ...
  if (options.enableRamping == BT_TRUE) {
    for (i = 0; i < nV; ++i) {
      t = static_cast<real_t>((i + rampOffset) % nRamp) /
...
returnValue QProblemB::solveInitialQP(const real_t* const xOpt,
  ...
  if (obtainAuxiliaryWorkingSet(xOpt, yOpt, guessedBounds, &auxiliaryBounds) !=
      SUCCESSFUL_RETURN)
    return THROWERROR(RET_INIT_FAILED);
  ...
  returnValue returnvalue =
      hotstart(g_original, lb_original, ub_original, nWSR, cputime);
...
returnValue QProblemB::solveQP(const real_t* const g_new,
  ...
  for (iter = nWSRperformed; iter < nWSR; ++iter) {
    ...
    returnvalue = determineDataShift(g_new, lb_new, ub_new, delta_g, delta_lb,
    ...
    returnvalue =
        determineStepDirection(delta_g, delta_lb, delta_ub, Delta_bB_isZero,
     ...
     * working set). */
    returnvalue = performStep(delta_g, delta_lb, delta_ub, delta_xFX, delta_xFR,
                              delta_yFX, BC_idx, BC_status);
    if (returnvalue != SUCCESSFUL_RETURN) {
      delete[] delta_yFX;
      delete[] delta_xFX;
      delete[] delta_xFR;
    ...
    if (homotopyLength <= options.terminationTolerance) {
      ...
      if (printIteration(iter, BC_idx, BC_status, homotopyLength,
    ...
    /* 7) Perform Ramping Strategy on zero homotopy step or drift correction (if
     * desired). */
    ...
    if (printIteration(iter, BC_idx, BC_status, homotopyLength, isFirstCall) !=
        SUCCESSFUL_RETURN)
      THROWERROR(
          RET_PRINT_ITERATION_FAILED); /* do not pass this as return value! */
  }
...
returnValue QProblemB::solveRegularisedQP(const real_t* const g_new,
  ...
  if (cputime == 0) {
    ...
    cputime_cur = *cputime;
    returnvalue = solveQP(g_new, lb_new, ub_new, nWSR, &cputime_cur,
  ...
  for (step = 0; step < options.numRegularisationSteps; ++step) {
    ...
    if (cputime == 0) {
      ...
      returnvalue = solveQP(gMod, lb_new, ub_new, nWSR, &cputime_cur,
 ...
 */
returnValue
QProblemB::setupAuxiliaryWorkingSet(const Bounds* const auxiliaryBounds,
 ...
 */
returnValue QProblemB::determineStepDirection(
    const real_t* const delta_g, const real_t* const delta_lb,
    const real_t* const delta_ub, BooleanType Delta_bB_isZero,
    real_t* const delta_xFX, real_t* const delta_xFR, real_t* const delta_yFX) {
  int_t i, ii;
  int_t r;
  int_t nFR = getNFR();
  int_t nFX = getNFX();
...
returnValue QProblemB::performStep(const real_t* const delta_g,
                                   ...
                                   const real_t* const delta_yFX, int_t& BC_idx,
                                   SubjectToStatus& BC_status) {
  int_t i, ii;
  int_t nV = getNV();
  int_t nFR = getNFR();
  ...
  if (BC_status == ST_UNDEFINED)
    ...
    snprintf(messageString, MAX_STRING_LENGTH,
             "Stepsize is %.15e! (idx = %d, status = %d)", tau, (int)BC_idx,
             (int)BC_status);
 ...
 */
BooleanType
QProblemB::shallRefactorise(const Bounds* const guessedBounds) const {
  int_t i;
  int_t nV = getNV();
 ...
 */
returnValue QProblemB::removeBound(int_t number, BooleanType updateCholesky) {
  int_t i;
  int_t nV = getNV();
  int_t nFR = getNFR();
 ...
 */
returnValue QProblemB::printIteration(int_t iter, int_t BC_idx,
                                      SubjectToStatus BC_status,
                                      real_t homotopyLength,
                                      BooleanType isFirstCall) {
#ifndef __SUPPRESSANYOUTPUT__
