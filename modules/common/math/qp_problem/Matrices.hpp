 ...
 *	\date 2009-2017
 *
 *  Various matrix classes: Abstract base matrix class, dense and sparse
 *matrices, including symmetry exploiting specializations.
 */
...
#include "modules/common/math/qp_problem/Indexlist.hpp"

BEGIN_NAMESPACE_QPOASES

/**
 *	\brief Abstract base class for interfacing tailored matrix-vector
 *operations.
 *
...
public:
   ...
   */
  virtual real_t
  getNorm(int_t type = 2 /**< Norm type, 1: one-norm, 2: Euclidean norm. */
          ) const = 0;
   ...
   */
  virtual real_t
  getRowNorm(int_t rNum,    /**< Row number. */
  ...
  virtual returnValue
  getRowNorm(real_t* norm,  /**< Norm of each row. */
             int_t type = 2 /**< Norm type, 1: one-norm, 2: Euclidean norm. */
             ) const = 0;
   ...
   *	\return SUCCESSFUL_RETURN */
  virtual returnValue
  getCol(int_t cNum,                   /**< Column number. */
  ...
  virtual returnValue getSparseSubmatrix(
      ...
      int_t* irn,    /**< Row position of entries (as position in irows) plus
                        rowoffset. */
      int_t* jcn,    /**< Column position of entries (as position in irows) plus
                        coloffset. */
      real_t* avals, /**< Numerical values of the entries. */
      BooleanType only_lower_triangular =
          BT_FALSE /**< if true, only the lower triangular portion is returned.
                      This can only be true for symmetric matrices and if
                      irows==jcols. */
      ) const;
   ...
   *	\return SUCCESSFUL_RETURN */
  virtual returnValue
  times(int_t xN,        /**< Number of vectors to multiply. */
   ...
   *	\return SUCCESSFUL_RETURN */
  virtual returnValue
  transTimes(int_t xN,        /**< Number of vectors to multiply. */
             real_t alpha,    /**< Scalar factor for matrix vector product. */
             const real_t* x, /**< Input vector to be multiplied. */
             int_t xLD,       /**< Leading dimension of input x. */
             real_t beta,     /**< Scalar factor for y. */
   ...
   *	\return SUCCESSFUL_RETURN */
  virtual returnValue
  times(const Indexlist* const irows, /**< Index list specifying rows. */
        const Indexlist* const icols, /**< Index list specifying columns. */
   ...
   *	\return SUCCESSFUL_RETURN */
  virtual returnValue transTimes(
      const Indexlist* const irows, /**< Index list specifying rows. */
      const Indexlist* const icols, /**< Index list specifying columns. */
      int_t xN,                     /**< Number of vectors to multiply. */
      real_t alpha,    /**< Scalar factor for matrix vector product. */
   ...
   *	\return SUCCESSFUL_RETURN */
  virtual returnValue writeToFile(FILE* output_file,
...
protected:
  BooleanType freeMemory; /**< Indicating whether internal memory needs to be
                             de-allocated. */
};

/**
 *	\brief Abstract base class for interfacing matrix-vector operations
 *tailored to symmetric matrices.
 *
 *	Abstract base class for symmetric matrices. Extends Matrix interface
 *with bilinear form evaluation.
 *
 *	\author Andreas Potschka, Christian Kirches, Hans Joachim Ferreau
 *	\version 3.2
 *	\date 2011-2017
...
/**
 *	\brief Interfaces matrix-vector operations tailored to general dense
 *matrices.
 *
...
/**
 *	\brief Interfaces matrix-vector operations tailored to symmetric dense
 *matrices.
 *
...
/**
 *	\brief Interfaces matrix-vector operations tailored to general sparse
 *matrices.
 *
...
protected:
  ...
  sparse_int_t* jd; /**< Indices to first entry of lower triangle (including
                       diagonal) (nCols). */
...
protected:
  ...
  sparse_int_t* jd; /**< Indices to first entry of upper triangle (including
                       diagonal) (nRows). */
  real_t* val;      /**< Vector of entries (length). */
};

/**
 *	\brief Interfaces matrix-vector operations tailored to symmetric sparse
 *matrices.
 *
