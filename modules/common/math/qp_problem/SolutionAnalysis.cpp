...
#include "modules/common/math/qp_problem/extras/SolutionAnalysis.hpp"
...
real_t SolutionAnalysis::getKktViolation(QProblemB* const qp,
  ...
  returnValue returnvalue = REFER_NAMESPACE_QPOASES getKktViolation(
      nV, H_ptr, qp->g, qp->lb, qp->ub, qp->x, qp->y, stat, feas, cmpl,
...
real_t SolutionAnalysis::getKktViolation(QProblem* const qp,
  ...
  returnValue returnvalue = REFER_NAMESPACE_QPOASES getKktViolation(
      nV, nC, H_ptr, qp->g, A_ptr, qp->lb, qp->ub, qp->lbA, qp->ubA, qp->x,
      qp->y, stat, feas, cmpl, workingSetB, workingSetC, hasIdentityHessian);
 ...
 */
returnValue
SolutionAnalysis::getVarianceCovariance(QProblemB* const qp,
                                        const real_t* const g_b_bA_VAR,
                                        real_t* const Primal_Dual_VAR) const {
 ...
 */
returnValue
SolutionAnalysis::getVarianceCovariance(QProblem* qp,
                                        const real_t* const g_b_bA_VAR,
                                        real_t* const Primal_Dual_VAR) const {
  ...
  for (run3 = 0; run3 < dim; run3++) {
    ...
    for (run1 = 0; run1 < nV; run1++) {
      ...
      delta_lb_cov[run1] =
          g_b_bA_VAR[run3 * dim + nV +
                     run1]; /*  LINE-WISE LOADING OF THE INPUT */
      delta_ub_cov[run1] =
          g_b_bA_VAR[run3 * dim + nV + run1]; /*  VARIANCE-COVARIANCE */
    }
    ...
    returnvalue = qp->determineStepDirection(
        delta_g_cov, delta_lbA_cov, delta_ubA_cov, delta_lb_cov, delta_ub_cov,
        Delta_bC_isZero, Delta_bB_isZero, delta_xFX, delta_xFR, delta_yAC,
        delta_yFX);
  ...
  for (run3 = 0; run3 < dim; run3++) {
    ...
    for (run1 = 0; run1 < nV; run1++) {
      ...
      delta_lb_cov[run1] =
          K[run3 + (nV + run1) * dim]; /*  ROW WISE LOADING OF THE */
      delta_ub_cov[run1] =
          K[run3 + (nV + run1) * dim]; /*  INTERMEDIATE RESULT K   */
    }
    for (run1 = 0; run1 < nC; run1++) {
 ...
 */
returnValue
SolutionAnalysis::getVarianceCovariance(SQProblem* const qp,
                                        const real_t* const g_b_bA_VAR,
                                        real_t* const Primal_Dual_VAR) const {
 ...
 */
returnValue
SolutionAnalysis::checkCurvatureOnStronglyActiveConstraints(SQProblem* qp) {
  printf("checkCurvatureOnStronglyActiveConstraints( SQProblem* qp ) not yet "
         "implemented for standard qpOASES!\n");
  return RET_INERTIA_CORRECTION_FAILED;
}

/*
 *	c h e c k C u r v a t u r e O n S t r o n g l y A c t i v e C o n s t r
 *a i n t s
 */
returnValue SolutionAnalysis::checkCurvatureOnStronglyActiveConstraints(
    SQProblemSchur* qp) {
  real_t eps = 1.0e-16;
  returnValue ret;
  Bounds saveBounds;
  ...
  // If no bounds are active reduced Hessian is positive definite (otherwise
  // qpOASES wouldnt have finished)
  if (nFX == 0)
    return SUCCESSFUL_RETURN;
  ...
  // If a variable is active now but has not been in the previous major
  // iteration remove it
  for (k = 0; k < nFX; k++)
...
// int_t SolutionAnalysis::checkCurvatureOnStronglyActiveConstraints(
// SQProblemSchur* qp )
//{
// real_t eps = 1.0e-16;
...
// if( qp->nS == oldNS + 1 )
//{
//// If the determinant does not change sign, then S has gained a positive
/// eigenvalue. / That means there is a negative eigenvalue in the (extended)
/// reduced Hessian!
...
//// That means there is a negative eigenvalue in the (extended) reduced
/// Hessian!
// if ( ( oldDet <= 0.0 && newDet > 0.0 ) || ( oldDet >= 0.0 && newDet < 0.0 ) )
//{
// fail = 1;
...
//// If not, don't bother with that because we will discard this QP object
/// anyway
// if( fail == 0 )
// for( k=0; k<nFX; k++ )
//{
// ret = qp->addBound( FX_idx[k], saveBounds.getStatus( FX_idx[k] ), BT_TRUE,
// BT_FALSE ); if( ret != SUCCESSFUL_RETURN && ret != RET_BOUND_ALREADY_ACTIVE )
